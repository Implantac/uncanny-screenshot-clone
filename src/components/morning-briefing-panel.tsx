      ...
      <CardHeader>
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div>
            ...
            <CardDescription>Snapshot operacional + 3 ações prioritárias para hoje</CardDescription>
          </div>
      ...
      <CardContent className="space-y-3">
          ...
          <>
            <div className="flex flex-wrap gap-1.5 text-xs">
              <Badge variant="outline" className="gap-1">
                ...
                {m.data.snapshot.productionOpen} OPs
              </Badge>
              <Badge
                variant={m.data.snapshot.productionLate > 0 ? "destructive" : "outline"}
                className="gap-1"
              >
                <AlertTriangle className="h-3 w-3" />
                {m.data.snapshot.productionLate} atrasadas
              </Badge>
              <Badge variant="outline" className="gap-1">
                <Package className="h-3 w-3" />
                {m.data.snapshot.prototypesPending} protótipos parados
              </Badge>
              <Badge variant="outline" className="gap-1">
                <ShieldCheck className="h-3 w-3" />
                {m.data.snapshot.capaOpen} CAPAs
              </Badge>
              <Badge variant="outline" className="gap-1">
                <Rocket className="h-3 w-3" />
                {m.data.snapshot.launchingCollections} em lançamento
              </Badge>
              <Badge
                variant={m.data.snapshot.costAlerts > 0 ? "destructive" : "outline"}
                className="gap-1"
              >
                <DollarSign className="h-3 w-3" />
                {m.data.snapshot.costAlerts} custos acima
              </Badge>
              <Badge
                variant={m.data.snapshot.lowStock > 0 ? "destructive" : "outline"}
                className="gap-1"
              >
                <Boxes className="h-3 w-3" />
                {m.data.snapshot.lowStock} estoque baixo
              </Badge>
            </div>
        ...
        {m.isError && (
          <p className="text-sm text-destructive">
            Erro ao gerar briefing: {(m.error as Error).message}
          </p>
        )}
      </CardContent>
    </Card>
