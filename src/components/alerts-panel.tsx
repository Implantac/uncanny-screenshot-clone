      ...
      <div className="px-2 py-2 border-b border-border flex flex-wrap gap-1" role="tablist">
        {(Object.keys(CAT_LABEL) as Cat[]).map((k) => (
          ...
          >
            {CAT_LABEL[k]}
            {counts[k] > 0 && (
              <span className="tabular-nums opacity-80" data-testid={`count-${k}`}>
                {counts[k]}
              </span>
            )}
          </button>
        ...
        {show("marketing") &&
          data.marketing.map((m) => (
            <LinkAs
              key={`mkt-${m.id}`}
              to={m.link || "/marketing"}
              href={m.link || "/marketing"}
              onClick={() => onMarketingClick?.(m.id)}
              data-testid="item-marketing"
              className="flex gap-3 px-4 py-3 border-b border-border last:border-0"
            >
        ...
        {show("comentario") &&
          data.comments.map((c) => (
            <LinkAs
              ...
              href={c.kind === "proto" ? "/prototipos" : "/pcp-kanban"}
