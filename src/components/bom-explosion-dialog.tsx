      ...
      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
        ...
        {isLoading && (
          <div className="text-sm text-muted-foreground py-8 text-center">
            Calculando explosão de matéria-prima...
          </div>
        )}
        {error && (
        ...
        {data && (
          <div className="space-y-4">
            <div className="text-xs text-muted-foreground">
              {data.product_name ?? "—"} · Grade total: <strong>{fmt(data.order.quantity)}</strong>{" "}
              peças
              {data.tech_sheet_status && (
                <Badge variant="outline" className="ml-2">
                  ficha {data.tech_sheet_status}
                </Badge>
              )}
            </div>
              ...
              <>
                <div className="grid grid-cols-3 gap-2">
                  <Stat
                    ...
                    value={fmtMoney(data.totals.necessidade_total)}
                  />
                  <Stat
                    ...
                    <tbody>
                      {data.lines.map((l, i) => (
                        <tr key={i} className="border-t">
                          ...
                          <td className="p-2 text-right tabular-nums">
                            {l.saldo !== null ? fmt(l.saldo) : "—"}
                          </td>
                          <td
                            className={`p-2 text-right tabular-nums ${l.gap !== null && l.gap < 0 ? "text-destructive font-semibold" : ""}`}
                          >
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: "ok" | "danger" }) {
  return (
    <div
      className={`rounded-md border p-2.5 ${tone === "danger" ? "border-destructive/40 bg-destructive/5" : ""}`}
    >
