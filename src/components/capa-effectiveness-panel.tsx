  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Mede quantas CAPAs fechadas <strong>não tiveram reincidência</strong> do mesmo fornecedor
          nos {data?.window_days ?? 60} dias seguintes. Métrica gerencial — não burocrática.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {isLoading && (
          <div className="text-sm text-muted-foreground">
            Cruzando CAPAs fechadas × inspeções pós-fechamento...
          </div>
        )}
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <Stat
                ...
                label="Efetividade"
                value={fmtPct(data.efetividade_pct)}
                tone={
                  data.efetividade_pct >= 80 ? "ok" : data.efetividade_pct >= 60 ? "warn" : "danger"
                }
              />
              <Stat
                icon={<ShieldCheck className="size-3.5" />}
                label="Efetivas"
                value={String(data.efetivas)}
              />
              <Stat
                icon={<AlertCircle className="size-3.5" />}
                label="Reincidentes"
                value={String(data.reincidentes)}
                tone={data.reincidentes > 0 ? "danger" : "ok"}
              />
              <Stat
                icon={<Clock className="size-3.5" />}
                label="Em verificação"
                value={String(data.em_verificacao)}
              />
            </div>
            ...
            {data.por_fornecedor.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  Pior efetividade por fornecedor
                </div>
                    ...
                    <tbody>
                      {data.por_fornecedor.slice(0, 8).map((s) => (
                        <tr key={s.supplier_id} className="border-t">
                          ...
                          <td
                            className={`p-2 text-right tabular-nums ${s.reincidentes > 0 ? "text-destructive font-semibold" : ""}`}
                          >
                            {s.reincidentes}
                          </td>
                          <td className="p-2 text-right tabular-nums">
                            <Badge variant={s.efetividade_pct >= 80 ? "outline" : "destructive"}>
                              {fmtPct(s.efetividade_pct)}
                            </Badge>
                          </td>
                        </tr>
                      ))}
            ...
            {data.rows.filter((r) => r.status_efetividade === "reincidente").length > 0 && (
              <div className="space-y-1.5">
                <div className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  CAPAs reincidentes (ação imediata)
                </div>
                    ...
                    .map((r) => (
                      ...
                      >
                        ...
                        <div className="text-muted-foreground mt-0.5">
                          {r.supplier_name ?? "—"} · {r.reincidencias} reincidência(s) após
                          fechamento em {new Date(r.closed_at).toLocaleDateString("pt-BR")}
                        </div>
                      </div>
                    ))}
                </div>
