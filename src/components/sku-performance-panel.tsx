...
export function SkuPerformancePanel({ productId, variants, colors, sizes }: Props) {
  ...
  const stats = useMemo(() => {
    ...
    const colorAgg = new Map<
      string,
      { name: string; hex: string | null; qty: number; value: number }
    >();
        ...
        <div className="flex items-center gap-2">
          ...
          {stats.totalQty > 0 && (
            <Badge variant="outline" className="ml-2 text-[11px]">
              {stats.totalQty.toLocaleString("pt-BR")} un · R${" "}
              {totalVal.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
            </Badge>
          )}
        </div>
        <div className="text-xs text-muted-foreground">
          {stats.salesCount} vendas casadas por SKU
        </div>
      </div>
              ...
              <div key={`${c.name}-${i}`} className="space-y-1">
                <div className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {i === 0 && (
                      <Badge className="h-4 text-[9px] gap-0.5">
                ...
                <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(c.qty / maxColorQty) * 100}%` }}
                  />
                </div>
              </div>
            ))}
