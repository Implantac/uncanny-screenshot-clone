...
import { Bell, AlertTriangle, Clock, CheckCircle2, MessageSquare, Megaphone, Pause, Sparkles, Check, BellOff } from "lucide-react";
...
export function NotificationsBell() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: inv }, { data: ops }, { data: stuckOps }, { data: oldProtos }, { data: pComments }, { data: poComments }, { data: mkt }, { data: stages }, { data: dism }] = await Promise.all([
        ...
        user?.id
          ? supabase.from("alert_dismissals").select("alert_key, dismissed_until, resolved").eq("user_id", user.id)
          : Promise.resolve({ data: [] as Dismissal[] }),
      ]);
      const slaByStage = new Map<string, number>((stages ?? []).map((s: any) => [s.key, Number(s.sla_stuck_days ?? 3)]));
      const stuckFiltered = (stuckOps ?? []).filter((o: any) => {
      ...
      const dismissals = new Map<string, Dismissal>(((dism ?? []) as Dismissal[]).map((d) => [d.alert_key, d]));
  ...
  const dismiss = useMutation({
    mutationFn: async ({ key, mode }: { key: string; mode: "snooze" | "resolve" }) => {
      ...
      const payload = {
        ...
        dismissed_until: mode === "snooze" ? new Date(Date.now() + 24 * 3600 * 1000).toISOString() : null,
      };
  ...
  const Actions = ({ alertKey }: { alertKey: string }) => (
    <div className="flex gap-0.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
      <button
        type="button"
        title="Adiar 1 dia"
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); dismiss.mutate({ key: alertKey, mode: "snooze" }); }}
      ...
      <button
        ...
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); dismiss.mutate({ key: alertKey, mode: "resolve" }); }}
        className="size-6 grid place-items-center rounded hover:bg-background"
      >
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-96 p-0">
        <div className="px-4 py-3 border-b border-border">
          ...
          <div className="text-xs text-muted-foreground">{total} alerta{total === 1 ? "" : "s"} · passe o mouse para adiar/resolver</div>
        </div>
        ...
        <div className="max-h-96 overflow-y-auto">
          ...
          {show("estoque") && data?.critical.map((i) => (
            <div key={`inv-${i.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
          ...
          {show("marketing") && data?.marketing.map((m) => (
            <div key={`mkt-${m.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
              <Link to={m.link || "/marketing"} onClick={() => markRead(m.id)} className="flex gap-3 flex-1 min-w-0">
          ...
          {show("atraso") && data?.overdue.map((o) => (
            <div key={`op-${o.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
          ...
          {show("parado") && data?.stuck.map((o: any) => {
            ...
            return (
              <div key={`stuck-${o.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
          ...
          {show("proto") && data?.oldProtos.map((p: any) => {
            ...
            return (
              <div key={`proto-${p.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
          ...
          {show("comentario") && data?.comments.map((c) => (
            <div key={`cm-${c.id}`} className="group flex gap-3 px-4 py-3 hover:bg-muted border-b border-border last:border-0 items-start">
              <Link to={c.kind === "proto" ? "/prototipos" : "/pcp-kanban"} className="flex gap-3 flex-1 min-w-0">
