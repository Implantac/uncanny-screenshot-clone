...
export function SupplierRecommender({ rfqTitle }: { rfqTitle?: string | null }) {
    ...
    queryFn: async () => {
      const [s, p, i, c] = await Promise.all([
        ...
        supabase
          ...
          .select("id, supplier_id, status, expected_date, created_at, updated_at")
          .limit(2000),
        supabase
          .from("purchase_order_items")
          .select("purchase_order_id, unit_price, quantity")
          .limit(5000),
  ...
  const ranked: Score[] = useMemo(() => {
    ...
    const rows: Score[] = data.suppliers
      ...
      .map((s) => {
        ...
        const prices: number[] = [];
        supplierPOs.forEach((p) =>
          (itemsByPo.get(p.id) ?? []).forEach((it) => {
        ...
        const leadDays: number[] = supplierPOs
          ...
          .map((p) =>
            Math.round(
              (new Date(p.expected_date!).getTime() - new Date(p.created_at).getTime()) /
                86_400_000,
            ),
          )
          .filter((n) => n > 0 && n < 365);
        ...
        const onTimeArr = receivedPOs
          ...
          .map(
            (p) =>
              new Date(p.updated_at).getTime() <= new Date(p.expected_date!).getTime() + 86_400_000,
          );
        const onTimePct = onTimeArr.length
          ? (onTimeArr.filter(Boolean).length / onTimeArr.length) * 100
          : null;
  ...
  return (
    <div className="glass rounded-xl p-4">
      ...
      <p className="text-xs text-muted-foreground mb-3">
        Ranking por histórico (preço, prazo, pontualidade, certificações).
        {rfqTitle ? ` Para: "${rfqTitle}"` : ""}
      </p>
      ...
      {isLoading ? (
        ...
        <div className="text-xs text-muted-foreground py-3">
          Sem histórico de compras suficiente para recomendar.
        </div>
      ) : (
                ...
                return (
                  <tr key={r.supplier.id} className="border-t border-border">
                    ...
                    <td className="p-2 text-right tabular-nums">
                      {r.avgLeadDays === null ? "—" : `${Math.round(r.avgLeadDays)}d`}
                    </td>
