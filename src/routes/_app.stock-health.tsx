...
export const Route = createFileRoute("/_app/stock-health")({
...
async function load(): Promise<Item[]> {
  ...
  (sales ?? []).forEach((s) => { if (s.sku) soldBySku.set(s.sku, (soldBySku.get(s.sku) ?? 0) + s.quantity); });
  ...
  return (inv ?? []).map((i) => {
    ...
    const status: Item["status"] =
      balance <= 0 ? "ruptura" :
      sold30 === 0 && ageDays > 90 ? "morto" :
      balance < minimum ? "baixo" :
      coverage > 180 ? "excesso" : "ok";
    return {
      id: i.id, sku: i.sku, name: i.name, category: i.category,
      deposit: i.deposit, balance, minimum, sold30, coverage, ageDays, status,
    };
  }).sort((a, b) => {
  ...
  return (
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Saúde do estoque: ruptura, baixo, morto, excesso e cobertura por SKU.</p>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <KPI label="Itens" value={summary.total} icon={<Package className="size-4" />} />
        <KPI label="Ruptura" value={summary.ruptura} icon={<AlertTriangle className="size-4" />} tone="destructive" />
        <KPI label="Abaixo do mínimo" value={summary.baixo} icon={<AlertTriangle className="size-4" />} tone="warning" />
        <KPI label="Estoque morto" value={summary.morto} icon={<Skull className="size-4" />} tone="muted" />
        <KPI label="Excesso" value={summary.excesso} icon={<Boxes className="size-4" />} tone="primary" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!isLoading && items.length === 0 && <tr><td colSpan={10} className="p-8 text-center text-muted-foreground">Sem itens cadastrados.</td></tr>}
              {items.slice(0, 200).map((i) => (
                <tr key={i.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-3 py-2 text-xs text-muted-foreground capitalize">{i.category}</td>
                  ...
                  <td className="px-3 py-2 text-right">{i.coverage > 365 ? "∞" : `${i.coverage}d`}</td>
                  <td className="px-3 py-2 text-right text-xs text-muted-foreground">{i.ageDays}d</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function KPI({ label, value, icon, tone = "default" }: { label: string; value: number | string; icon: React.ReactNode; tone?: "default" | "destructive" | "warning" | "primary" | "muted" }) {
  const tones = { default: "", destructive: "text-destructive", warning: "text-warning", primary: "text-primary", muted: "text-muted-foreground" };
  return (
