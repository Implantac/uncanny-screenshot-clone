...
const normHandle = (s: string | null | undefined) =>
...
async function load(): Promise<Inf[]> {
  ...
  const [{ data: salesRows }, { data: erpRows }] = await Promise.all([
    ...
    supabase
      ...
      .select("influencer_code, total_value")
  ...
  return (infs ?? [])
    .map((i) => {
      ...
      const baseUplift =
        realUplift !== undefined ? realUplift : Number(i.vendas_depois) - Number(i.vendas_antes);
      ...
      return {
        ...
        engajamento: Number(i.engajamento),
        valor: Number(i.valor),
        vendas_antes: Number(i.vendas_antes),
        vendas_depois: Number(i.vendas_depois),
        uplift,
        upliftPct,
        roas,
        cpm,
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Atribuição, uplift e ROAS por influenciador.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="Investimento"
          value={fmt(totals.investment)}
          icon={<DollarSign className="size-4" />}
        />
        <KPI
          label="Uplift total"
          value={fmt(totals.uplift)}
          icon={<TrendingUp className="size-4" />}
          tone="success"
        />
        <KPI
          label="ROAS médio"
          value={`${totals.avgRoas.toFixed(2)}x`}
            ...
            <tbody>
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td
                    className={`px-3 py-2 text-right font-medium ${r.uplift >= 0 ? "text-success" : "text-destructive"}`}
                  >
                    {fmt(r.uplift)}
                  </td>
                  <td className="px-3 py-2 text-right">
                    {r.upliftPct > 0 ? "+" : ""}
                    {r.upliftPct.toFixed(0)}%
                  </td>
                  <td
                    className={`px-3 py-2 text-right font-semibold ${r.roas >= 3 ? "text-success" : r.roas >= 1 ? "text-warning" : "text-destructive"}`}
                  >
                    {r.roas.toFixed(2)}x
                  </td>
                </tr>
