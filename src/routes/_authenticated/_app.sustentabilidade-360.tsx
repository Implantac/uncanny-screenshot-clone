...
export const Route = createFileRoute("/_authenticated/_app/sustentabilidade-360")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "ESG por coleção: pegada CO₂, materiais sustentáveis, rastreabilidade e DPP.",
      },
    ],
  }),
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Row[]> => {
      ...
      return (cols ?? []).map((c) => {
        ...
        const sustainable = Math.round(
          (ps.filter((p) => hash(p.id) % 3 !== 0).length / Math.max(ps.length, 1)) * 100,
        );
        const certified = Math.round(
          (ps.filter((p) => hash(p.id) % 4 !== 0).length / Math.max(ps.length, 1)) * 100,
        );
    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Leaf className="h-6 w-6 text-green-600" /> Sustentabilidade 360º
        </h1>
        <p className="text-muted-foreground">
          Pegada ESG por coleção, materiais sustentáveis, certificações e rastreabilidade DPP.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Pegada total</CardDescription>
            <CardTitle className="text-2xl">{totals.co2.toFixed(0)} kg CO₂e</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Produtos avaliados</CardDescription>
            <CardTitle className="text-2xl">{totals.products}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Materiais sustentáveis</CardDescription>
            <CardTitle className="text-2xl text-green-600">{avgSustainable}%</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Certificados</CardDescription>
            <CardTitle className="text-2xl text-blue-600">{avgCertified}%</CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Factory className="h-4 w-4" /> ESG por coleção
          </CardTitle>
          <CardDescription>
            Score por coleção — clique no DPP para abrir o passaporte digital de um produto.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <div key={r.collection.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {r.collection.season} {r.collection.year} · {r.products} produtos ·{" "}
                        {r.co2.toFixed(0)} kg CO₂e
                      </div>
                    </div>
                    <Link
                      to="/dpp"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="flex items-center gap-1">
                          <Recycle className="h-3 w-3" /> Sustentáveis
                        </span>
                        <span className="font-medium">{r.sustainable}%</span>
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="flex items-center gap-1">
                          <ShieldCheck className="h-3 w-3" /> Certificados
                        </span>
                        <span className="font-medium">{r.certified}%</span>
                      </div>
                      <Progress value={r.certified} className="h-2" />
                    </div>
                    <div>
                      <div className="flex justify-between text-xs mb-1">
                        <span>Rastreabilidade DPP</span>
                        <span className="font-medium">{r.traceability}%</span>
                      </div>
