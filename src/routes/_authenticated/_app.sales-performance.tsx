...
async function load() {
  ...
  const { data } = await supabase
    .from("sales")
    .select("product_id, sku, channel, quantity, total, sold_at")
    .gte("sold_at", since);
  return (data ?? []) as Sale[];
}
...
function SalesPerformance() {
  ...
  const byDay = useMemo(() => {
    ...
    const days = Array.from(m.entries())
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Receita, canais e top SKUs nos últimos 90 dias.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="Peças"
          value={totals.qty.toLocaleString("pt-BR")}
          icon={<ShoppingBag className="size-4" />}
        />
        <KPI
          label="Ticket médio"
          value={fmt(totals.ticket)}
              ...
              <div key={c.channel} className="px-4 py-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span>
                    ...
                    <span className="text-xs text-muted-foreground">
                      ({totalChan > 0 ? Math.round((c.revenue / totalChan) * 100) : 0}%)
                    </span>
                  </span>
                </div>
                <div className="h-1.5 mt-2 bg-muted rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${totalChan > 0 ? (c.revenue / totalChan) * 100 : 0}%` }}
                  />
                </div>
                <div className="text-xs text-muted-foreground mt-1">
                  {c.orders} pedidos • {c.qty} peças
                </div>
              </div>
            ))}
          </div>
