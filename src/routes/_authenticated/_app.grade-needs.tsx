...
export const Route = createFileRoute("/_authenticated/_app/grade-needs")({
  head: () => ({
    meta: [
    ...
    return products
      .map((p) => {
        ...
        sizes.forEach((sz) => {
          needs[sz] =
            total > 0
              ? Math.round((bySize[sz] / total) * target)
              : Math.round(target / sizes.length);
        });
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Quebra PP/P/M/G/GG por SKU baseada no histórico real de vendas.
        </p>
      </header>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <KPI
          ...
          value={summary.suggested.toLocaleString("pt-BR")}
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-muted-foreground text-xs">
            <tr>
              ...
              {allSizes.map((sz) => (
                <th key={sz} className="text-right px-3 py-2">
                  {sz}
                </th>
              ))}
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={allSizes.length + 2} className="p-8 text-center text-muted-foreground">
                  Carregando…
                </td>
              </tr>
            ) : rows.length === 0 ? (
