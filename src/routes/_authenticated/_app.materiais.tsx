...
type Item = {
  id: string; kind: string; code: string; name: string; composition: string | null;
  color_hex: string | null; unit: string | null; reference_cost: number; active: boolean;
};
...
function Page() {
  ...
  const [form, setForm] = useState({ kind: "tecido", code: "", name: "", composition: "", color_hex: "", unit: "m", reference_cost: 0 });

  const items = useQuery({
    queryKey: ["material-library", filter],
    queryFn: async () => {
      let q = (supabase as any).from("material_library").select("*").order("created_at", { ascending: false });
  ...
  const add = useMutation({
    mutationFn: async () => {
      const { error } = await (supabase as any).from("material_library").insert({ owner_id: user!.id, ...form });
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Material adicionado"); setForm({ ...form, code: "", name: "", composition: "" }); qc.invalidateQueries({ queryKey: ["material-library"] }); },
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Tecidos, aviamentos e cores reutilizáveis entre coleções.</p>
        </div>
      </div>
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
          ...
          <Input type="color" value={form.color_hex || "#000000"} onChange={(e) => setForm({ ...form, color_hex: e.target.value })} />
          <Input placeholder="Unid." value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })} />
          <Input type="number" placeholder="Custo ref." value={form.reference_cost} onChange={(e) => setForm({ ...form, reference_cost: Number(e.target.value) })} />
        </div>
        ...
        {items.data?.map((it) => (
          <div key={it.id} className="glass rounded-xl p-4 flex flex-col gap-2">
            <div className="flex items-start justify-between">
              ...
              {it.color_hex && <div className="w-8 h-8 rounded-full border" style={{ background: it.color_hex }} />}
            </div>
            {it.composition && <p className="text-sm">{it.composition}</p>}
            <div className="flex items-center justify-between mt-auto pt-2">
              <span className="text-sm font-medium">R$ {Number(it.reference_cost).toFixed(2)}/{it.unit}</span>
              <Button size="icon" variant="ghost" onClick={() => remove.mutate(it.id)}><Trash2 className="h-4 w-4" /></Button>
            </div>
          </div>
        ))}
        {items.data?.length === 0 && <p className="text-muted-foreground col-span-full text-center py-8">Nenhum material cadastrado.</p>}
      </div>
    </div>
  );
}
