...
async function loadCollection(id: string) {
  const [col, prods, protos, orders, sales, sheets] = await Promise.all([
    ...
    supabase
      ...
      .select("id, product_id, stage, status, quantity, due_date")
...
function WarRoomColecao() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["war-room-colecao", id],
    queryFn: () => loadCollection(id),
  });

  const c = data?.collection;
  const productIds = useMemo(() => new Set((data?.products ?? []).map((p) => p.id)), [data]);
  const cProtos = (data?.prototypes ?? []).filter(
    (p) => p.product_id && productIds.has(p.product_id),
  );
  ...
  const inDev = (data?.products ?? []).filter(
    (p) => !p.status || /dev|brief|model|prot/i.test(p.status),
  );
  ...
  const atrasadas = cOrders.filter(
    (o) => o.due_date && new Date(o.due_date).getTime() < Date.now() && o.stage !== "entregue",
  );

  const salesByProduct = new Map<string, number>();
  cSales.forEach((s) => {
    if (s.product_id)
      salesByProduct.set(s.product_id, (salesByProduct.get(s.product_id) ?? 0) + s.quantity);
  });
  const ranked = [...salesByProduct.entries()].sort((a, b) => b[1] - a[1]);
  const campeoes = ranked
    .slice(0, 3)
    .map(([pid, qty]) => ({ p: data?.products.find((x) => x.id === pid), qty }));
  const criticos = (data?.products ?? []).filter((p) => !salesByProduct.get(p.id)).slice(0, 3);

  if (isLoading)
    return <div className="p-8 text-sm text-muted-foreground">Carregando War Room…</div>;
  ...
  const cards = [
    ...
    },
    {
      ...
      value: pilotosPend.length,
      icon: Scissors,
      to: "/pilots",
      tone: "warning",
    },
    {
      label: "Produtos sem ficha aprovada",
      value: semFicha.length,
      icon: FileWarning,
      to: "/ficha-tecnica",
      tone: "warning",
    },
    {
      label: "Pilotos liberados",
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5">
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <Link to="/colecao-360" className="text-xs text-primary hover:underline">
          Ver Coleção 360º →
        </Link>
      </div>
  ...
  if (!launchDate) {
    return (
      <div className="glass rounded-xl p-4 flex items-center gap-3 text-sm text-muted-foreground">
        <Timer className="size-4" /> Defina uma data de lançamento na coleção para ativar o
        countdown.
      </div>
    );
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs opacity-80">
            Lançamento previsto: {new Date(launchDate).toLocaleDateString("pt-BR")}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
        {items.map((i) => (
          <Link
            key={i.label}
            to={i.to}
            className="rounded-lg bg-background/60 border border-destructive/20 px-3 py-2 hover:border-destructive/50 transition-colors"
          >
