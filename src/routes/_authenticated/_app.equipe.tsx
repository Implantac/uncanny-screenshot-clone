      ...
      <div className="p-8 max-w-xl">
        <div className="glass rounded-2xl p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Apenas administradores podem gerenciar a equipe e permissões.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <table className="w-full text-sm min-w-[900px]">
          <thead className="bg-muted/30 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="px-2 py-2 text-center border-l border-border" colSpan={ROLES.length}>
                Papéis
              </th>
              <th
                className="px-2 py-2 text-center border-l border-border"
                colSpan={APP_SECTORS.length}
              >
                Setores (telas)
              </th>
            </tr>
            <tr>
              {ROLES.map((r, i) => (
                <th
                  key={r}
                  className={`px-2 py-2 text-center ${i === 0 ? "border-l border-border" : ""}`}
                >
                  {ROLE_LABEL[r]}
                </th>
              ))}
              ...
              <tr key={u.id} className="border-t border-border">
                ...
                {ROLES.map((r, i) => {
                  ...
                  return (
                    <td
                      key={r}
                      className={`px-2 py-3 text-center ${i === 0 ? "border-l border-border" : ""}`}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        disabled={busy}
                        onChange={(e) =>
                          mSector.mutate({ userId: u.id, sector: s, enabled: e.target.checked })
                        }
                        className="size-4 accent-primary cursor-pointer"
                      />
                    </td>
      ...
      <div className="flex items-start gap-2 text-xs text-muted-foreground">
        ...
        <p>
          Setores controlam quais <strong>telas</strong> cada usuário enxerga no menu (Marketing,
          PCP, Desenvolvimento). Administradores veem tudo. Demais áreas (Operação, Coleções,
          Cadeia, Inteligência, ERP, Plataforma) são liberadas a todos os usuários autenticados.
        </p>
      </div>
    </div>
  );
