...
async function load() {
  const [{ data: orders }, { data: suppliers }] = await Promise.all([
    supabase
      ...
      .select("id, code, status, quantity, progress, due_date, supplier_id, suppliers(name)")
...
function Capacity() {
  ...
  const bySupplier = useMemo(() => {
    const m = new Map<
      string,
      { name: string; orders: number; qty: number; progress: number; late: number }
    >();
    orders.forEach((o) => {
      ...
      if (o.due_date && new Date(o.due_date).getTime() < today && o.status !== "concluida")
        c.late += 1;
      m.set(key, c);
    });
    return Array.from(m.values())
      .map((s) => ({ ...s, avgProgress: s.orders > 0 ? s.progress / s.orders : 0 }))
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <KPI
          label="OEE médio"
          value={`${summary.oee.toFixed(0)}%`}
          icon={<Gauge className="size-4" />}
          tone={summary.oee >= 70 ? "success" : summary.oee >= 40 ? "warning" : "destructive"}
        />
        <KPI
          label="Atrasadas"
          value={summary.late}
      ...
      <div
        className={`rounded-xl border p-4 ${overloaded.length ? "border-warning/50 bg-warning/5" : "border-border bg-card"}`}
      >
        <div className="flex items-center gap-2 text-sm font-medium">
          <Activity className="size-4 text-primary" /> Leitura do PCP
        </div>
              ...
              <div key={s.name} className="px-4 py-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {s.orders} ordens • {s.qty.toLocaleString("pt-BR")} peças
                    {s.late > 0 && (
                      <span className="ml-2 text-destructive">• {s.late} atrasadas</span>
                    )}
                  </span>
                </div>
                <div className="h-2 mt-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className={`h-full ${s.avgProgress >= 70 ? "bg-success" : s.avgProgress >= 40 ? "bg-warning" : "bg-primary"}`}
                    style={{ width: `${s.avgProgress}%` }}
                  />
                </div>
                <div className="text-[10px] text-muted-foreground mt-1">
                  {s.avgProgress.toFixed(0)}% médio
                </div>
              </div>
            ))}
          </div>
                ...
                .map((o) => {
                  ...
                  return (
                    <tr key={o.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      <td
                        className={`px-3 py-2 text-xs ${late ? "text-destructive font-medium" : "text-muted-foreground"}`}
                      >
