...
async function load() {
  const [{ data: products }, { data: collections }, { data: sales }, { data: campaigns }] =
    await Promise.all([
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Lucro real por coleção: receita − CMV − marketing.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="CMV + Marketing"
          value={`R$ ${(summary.cogs + summary.mkt).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={<TrendingDown className="size-4" />}
        />
        <KPI
          label="Lucro líquido"
          value={`R$ ${summary.net.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={<TrendingUp className="size-4" />}
          tone={summary.net >= 0 ? "success" : "destructive"}
        />
        <KPI
          label="ROI global"
          value={`${summary.roi.toFixed(0)}%`}
          icon={<Layers className="size-4" />}
          tone={summary.roi >= 0 ? "success" : "destructive"}
        />
      </div>
              ...
              byCollection.map((c) => (
                <tr key={c.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">
                    R$ {c.cogs.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">
                    R$ {c.mkt.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums font-semibold ${c.netProfit >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"}`}
                  >
                    R$ {c.netProfit.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
