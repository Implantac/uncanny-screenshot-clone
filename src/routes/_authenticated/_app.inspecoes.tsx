...
export const Route = createFileRoute("/_authenticated/_app/inspecoes")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Inspeções AQL por OP/OS/protótipo com defeitos críticos, maiores e menores." },
    ],
  }),
...
type Inspection = {
  id: string; inspection_type: string; aql_level: string | null;
  lot_size: number | null; sample_size: number | null;
  critical_defects: number; major_defects: number; minor_defects: number;
  result: string; inspector: string | null; notes: string | null;
  inspected_at: string; supplier_id: string | null;
  production_order_id: string | null; service_order_id: string | null; prototype_id: string | null;
};
...
function InspectionsPage() {
  ...
  const insps = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any)
        .from("quality_inspections").select("*").order("inspected_at", { ascending: false });
  ...
  const pos = useQuery({
    queryKey: ["insp-pos"],
    queryFn: async () => {
      const { data } = await supabase.from("production_orders").select("id, code").order("created_at", { ascending: false }).limit(100);
    ...
    <div className="p-6 space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Plano AQL por ordem, com classificação de defeitos e laudo.</p>
        </div>
        <Button onClick={() => setOpen(true)}><Plus className="size-4 mr-1" /> Nova inspeção</Button>
      </header>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {insps.isLoading ? (
              ...
              <tr><td colSpan={9} className="p-8 text-center text-muted-foreground">Nenhuma inspeção registrada.</td></tr>
            ) : (insps.data ?? []).map((i) => {
              const po = pos.data?.find((p) => p.id === i.production_order_id);
              return (
                <tr key={i.id} className="border-t border-border">
                  <td className="px-3 py-2">{new Date(i.inspected_at).toLocaleDateString("pt-BR")}</td>
                  ...
                  <td className="px-3 py-2">{(i.lot_size ?? "—") + " / " + (i.sample_size ?? "—")}</td>
...
function NewInspectionDialog({ open, onOpenChange, userId, suppliers, pos, onSaved }: {
  open: boolean; onOpenChange: (v: boolean) => void; userId: string;
  suppliers: { id: string; name: string }[]; pos: { id: string; code: string }[]; onSaved: () => void;
}) {
  const [f, setF] = useState({
    inspection_type: "final", production_order_id: "", supplier_id: "",
    aql_level: "II", lot_size: "", sample_size: "",
    critical_defects: "0", major_defects: "0", minor_defects: "0",
    result: "pendente", inspector: "", notes: "",
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success("Inspeção registrada"); onSaved(); onOpenChange(false); },
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Select value={f.inspection_type} onValueChange={(v) => setF({ ...f, inspection_type: v })}>
          ...
          <div>
            ...
            <Select value={f.aql_level} onValueChange={(v) => setF({ ...f, aql_level: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>{AQL.map((a) => <SelectItem key={a} value={a}>{a}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Ordem de Produção</label>
            <Select value={f.production_order_id} onValueChange={(v) => setF({ ...f, production_order_id: v })}>
          ...
          <div>
            ...
            <Input type="number" value={f.lot_size} onChange={(e) => setF({ ...f, lot_size: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Amostra</label>
            <Input type="number" value={f.sample_size} onChange={(e) => setF({ ...f, sample_size: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Defeitos críticos</label>
            <Input type="number" value={f.critical_defects} onChange={(e) => setF({ ...f, critical_defects: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Defeitos maiores</label>
            <Input type="number" value={f.major_defects} onChange={(e) => setF({ ...f, major_defects: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-muted-foreground">Defeitos menores</label>
            <Input type="number" value={f.minor_defects} onChange={(e) => setF({ ...f, minor_defects: e.target.value })} />
          </div>
          <div>
          ...
          <div className="col-span-2">
            ...
            <Input value={f.inspector} onChange={(e) => setF({ ...f, inspector: e.target.value })} />
          </div>
          <div className="col-span-2">
