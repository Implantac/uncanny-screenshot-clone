...
export const Route = createFileRoute("/_authenticated/_app/attribution")({ component: Attribution });
...
function labelFor(c: string) { return CHANNEL_LABELS[c]?.label ?? c; }
function colorFor(c: string) { return CHANNEL_LABELS[c]?.color ?? "bg-muted-foreground"; }
...
function Attribution() {
  ...
  const byChannel = useMemo(() => {
    const m = new Map<string, { channel: string; revenue: number; orders: number; units: number }>();
    sales.forEach((s) => {
  ...
  const infUplift = influencers.reduce((s, i) => s + (Number(i.vendas_depois) - Number(i.vendas_antes)), 0);
  ...
  return (
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Origem da receita por canal: e-commerce, social, influenciadores, marketplace, representantes.</p>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI label="Receita total" value={`R$ ${total.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`} icon={<TrendingUp className="size-4" />} tone="primary" />
        <KPI label="Canais ativos" value={byChannel.length} icon={<RouteIcon className="size-4" />} />
        <KPI label="Uplift de influenciadores" value={`R$ ${infUplift.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`} icon={<Users className="size-4" />} tone="success" />
        <KPI label="ROI influenciadores" value={infInvest > 0 ? `${((infUplift / infInvest) * 100).toFixed(0)}%` : "—"} icon={<TrendingUp className="size-4" />} />
      </div>

      <div className="rounded-xl border border-border bg-card p-4 space-y-3">
        <div className="font-medium">Mix de receita por canal</div>
        {total === 0 ? <div className="text-sm text-muted-foreground py-6 text-center">Sem vendas registradas.</div> : (
          <>
            <div className="flex w-full h-3 rounded-full overflow-hidden bg-muted">
              {byChannel.map((c) => (
                <div key={c.channel} className={colorFor(c.channel)} style={{ width: `${(c.revenue / total) * 100}%` }} title={labelFor(c.channel)} />
              ))}
            </div>
            <div className="flex flex-wrap gap-3 text-xs">
              {byChannel.map((c) => (
                <div key={c.channel} className="flex items-center gap-1.5">
                  ...
                  <span>{labelFor(c.channel)} · {((c.revenue / total) * 100).toFixed(1)}%</span>
                </div>
              ))}
            </div>
          </>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">Carregando…</td></tr> :
              byChannel.length === 0 ? <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">Sem dados.</td></tr> :
              byChannel.map((c) => (
                <tr key={c.channel} className="border-t border-border">
                  <td className="px-3 py-2"><div className="flex items-center gap-2"><span className={`size-2.5 rounded-full ${colorFor(c.channel)}`} />{labelFor(c.channel)}</div></td>
                  ...
                  <td className="px-3 py-2 text-right tabular-nums text-muted-foreground">R$ {c.orders > 0 ? (c.revenue / c.orders).toFixed(0) : "0"}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{((c.revenue / total) * 100).toFixed(1)}%</td>
                </tr>
              ))}
          </tbody>
        </table>
