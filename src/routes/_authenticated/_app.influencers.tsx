...
function Influencers() {
  ...
  const summary = useMemo(() => {
    ...
    const uplift = influencers.reduce(
      (s, i) => s + (Number(i.vendas_depois) - Number(i.vendas_antes)),
      0,
    );
    const avgEng = influencers.length
      ? influencers.reduce((s, i) => s + Number(i.engajamento), 0) / influencers.length
      : 0;
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Cadastro completo de criadores, segmentos, cidades e uplift de vendas.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="Alcance total"
          value={summary.totalSeg.toLocaleString("pt-BR")}
          icon={<Instagram className="size-4" />}
          tone="primary"
        />
        <KPI
          label="Investido"
          value={`R$ ${summary.invest.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={<DollarSign className="size-4" />}
        />
        <KPI
          label="Uplift de vendas"
          value={`R$ ${summary.uplift.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={<TrendingUp className="size-4" />}
          tone={summary.uplift >= 0 ? "success" : "destructive"}
        />
      </div>
      ...
      <input
        ...
        className="w-full md:max-w-md px-3 py-2 text-sm bg-card border border-border rounded-lg"
      />
                ...
                return (
                  <tr key={i.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-2 text-right tabular-nums">
                      {i.seguidores.toLocaleString("pt-BR")}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">
                      {Number(i.engajamento).toFixed(1)}%
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums">
                      R$ {Number(i.valor).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                    </td>
                    <td
                      className={`px-3 py-2 text-right tabular-nums ${uplift >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-destructive"}`}
                    >
