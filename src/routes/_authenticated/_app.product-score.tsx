...
function ProductScore() {
  ...
  const rows = useMemo(() => {
    ...
    const maxUnits = Math.max(
      1,
      ...products.map((p) =>
        sales.filter((s) => s.product_id === p.id).reduce((a, b) => a + b.quantity, 0),
      ),
    );
    return products
      .map((p) => {
        ...
        const score = Math.round(
          vendasN * 0.4 +
            Math.min(100, margin * 1.5) * 0.25 +
            Math.min(100, roi) * 0.2 +
            Math.min(100, units * 2) * 0.15,
        );
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Pontuação 0–100 ponderando vendas, margem, ROI e giro.
        </p>
      </header>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <KPI
          label="Receita total"
          value={`R$ ${summary.revenue.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={<DollarSign className="size-4" />}
          tone="success"
        />
        <KPI
          label="Lucro estimado"
          value={`R$ ${summary.profit.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
              ...
              rows.slice(0, 50).map((r) => (
                <tr key={r.id} className="border-t border-border">
                  <td className="px-3 py-2">
                    <span
                      className={`inline-block min-w-[3rem] text-center text-xs font-semibold px-2 py-1 rounded ${r.score >= 70 ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : r.score >= 40 ? "bg-amber-500/15 text-amber-600 dark:text-amber-400" : "bg-muted text-muted-foreground"}`}
                    >
                      {r.score}
                    </span>
                  </td>
                  ...
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${r.profit < 0 ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"}`}
                  >
                    R$ {r.profit.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
