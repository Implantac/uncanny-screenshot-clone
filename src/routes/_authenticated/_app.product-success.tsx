...
async function load() {
  const [{ data: products }, { data: sales }] = await Promise.all([
    supabase
      .from("products")
      .select("id, sku, name, category, cost_price, sell_price, image_url, created_at"),
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Score preditivo 0–100 baseado em velocidade, margem e recência.
        </p>
      </header>
              ...
              scored.slice(0, 50).map((p) => (
                <tr key={p.id} className="border-t border-border">
                  <td className="px-3 py-2">
                    <span
                      className={`inline-block min-w-[3rem] text-center text-xs font-semibold px-2 py-1 rounded ${p.score >= 70 ? "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400" : p.score >= 40 ? "bg-amber-500/15 text-amber-600 dark:text-amber-400" : "bg-destructive/15 text-destructive"}`}
                    >
                      {p.score}
                    </span>
                  </td>
