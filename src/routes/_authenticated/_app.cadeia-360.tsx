...
export const Route = createFileRoute("/_authenticated/_app/cadeia-360")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Portal do fornecedor, sourcing inteligente e calendário de capacidade.",
      },
    ],
  }),
  component: Cadeia360,
...
async function loadAll() {
  const [suppliers, orders, products] = await Promise.all([
    supabase
      .from("suppliers")
      .select("id, name, category, rating, active")
      .eq("active", true)
      .limit(200),
    supabase
      .from("production_orders")
      .select(
        "id, code, quantity, stage, status, due_date, supplier_id, product_id, stage_updated_at, products(name, sku, category)",
      )
    ...
    return suppliers
      .map((s) => {
        ...
        const onTime = delivered.filter(
          (o) =>
            new Date(o.stage_updated_at).getTime() <= new Date(o.due_date!).getTime() + 86400000,
        ).length;
        ...
        return {
          ...
          score,
          categories: new Set(sOrders.map((o) => o.products?.category).filter(Boolean) as string[]),
        };
      })
    ...
    return products
      ...
      .map((p) => {
        const candidates = scored
          .filter(
            (s) =>
              !p.category || s.categories.has(p.category) || s.supplier.category === p.category,
          )
          .slice(0, 3);
      ...
      <section className="rounded-xl border border-border bg-card">
        ...
        {sourcing.length === 0 ? (
          <div className="p-6 text-sm text-muted-foreground">
            Todos os produtos aprovados estão em produção. ✅
          </div>
        ) : (
                  ...
                  {suggestion ? (
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        score {suggestion.score} · OTD {Math.round(suggestion.otd * 100)}%
                      </div>
                    </div>
                  ) : (
      ...
      <section className="rounded-xl border border-border bg-card">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2 font-medium">
            ...
            Calendário de capacidade (8 semanas)
          </div>
          <Link
            to="/capacity"
          ...
          <table className="w-full text-xs">
            <thead className="bg-muted/30">
              <tr>
                ...
                {heatmap.weeks.map((w) => (
                  <th key={w.label} className="text-center px-2 py-2 font-medium tabular-nums">
                    {w.label}
                  </th>
                ))}
              </tr>
                ...
                <tr key={r.name} className="border-t border-border">
                  ...
                  {r.cells.map((qty, i) => {
                    ...
                    const bg =
                      qty === 0
                        ? "transparent"
                        : `color-mix(in oklab, hsl(var(--primary)) ${Math.round(intensity * 70)}%, transparent)`;
                    return (
                      <td
                        key={i}
      ...
      <section className="rounded-xl border border-border bg-card">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          ...
          <Link
            ...
            className="text-xs text-muted-foreground hover:text-foreground inline-flex items-center gap-1"
          >
            Cadastro <ArrowRight className="size-3" />
          </Link>
        </div>
        <ul className="divide-y divide-border">
          {scored.slice(0, 10).map((s) => (
            <li
              key={s.supplier.id}
              className="px-4 py-3 grid grid-cols-12 items-center gap-2 text-sm"
            >
