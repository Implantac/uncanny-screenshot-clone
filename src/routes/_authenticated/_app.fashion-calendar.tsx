...
async function load(): Promise<Col[]> {
  const [{ data: cols }, { data: prods }] = await Promise.all([
    supabase.from("collections").select("id, name, season, year, status, progress, launch_date").order("launch_date", { ascending: true, nullsFirst: false }),
    supabase.from("products").select("collection_id"),
  ]);
  const countByCol = new Map<string, number>();
  (prods ?? []).forEach((p) => { if (p.collection_id) countByCol.set(p.collection_id, (countByCol.get(p.collection_id) ?? 0) + 1); });
  return (cols ?? []).map((c) => ({ ...c, products: countByCol.get(c.id) ?? 0 }));
}

function FashionCalendar() {
  const { data: cols = [], isLoading } = useQuery({ queryKey: ["fashion-calendar"], queryFn: load });
  const today = new Date();

  const enriched = useMemo(() => cols.map((c) => {
    ...
    const phase = c.status === "lancada" || c.status === "concluida" ? "lançada"
      : days != null && days < 0 ? "atrasada"
      : days != null && days < 30 ? "iminente"
      : days != null && days < 90 ? "produção"
      : "desenvolvimento";
    return { ...c, days, phase };
  }), [cols]);
  ...
  return (
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">Linha do tempo de coleções: desenvolvimento, produção, lançamento e sell-in.</p>
      </header>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI label="Coleções" value={summary.total} icon={<Package className="size-4" />} />
        <KPI label="Lançamento iminente" value={summary.iminente} icon={<Clock className="size-4" />} tone="warning" />
        <KPI label="Atrasadas" value={summary.atrasada} icon={<AlertTriangle className="size-4" />} tone="destructive" />
        <KPI label="Lançadas" value={summary.lancada} icon={<CheckCircle2 className="size-4" />} tone="success" />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        ...
        {isLoading ? <div className="p-8 text-center text-muted-foreground">Carregando…</div> : enriched.length === 0 ? (
            ...
            {enriched.map((c) => (
              <div key={c.id} className="px-4 py-3 grid grid-cols-12 gap-3 items-center text-sm">
                <div className="col-span-12 md:col-span-4">
                  ...
                  <div className="text-xs text-muted-foreground">{c.season} {c.year} • {c.products} produtos</div>
                </div>
                <div className="col-span-6 md:col-span-2">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                    c.phase === "atrasada" ? "bg-destructive/15 text-destructive" :
                    c.phase === "iminente" ? "bg-warning/15 text-warning" :
                    c.phase === "lançada" ? "bg-success/15 text-success" :
                ...
                <div className="col-span-6 md:col-span-3 text-xs text-muted-foreground">
                  ...
                  {c.days != null && <span className={`ml-2 ${c.days < 0 ? "text-destructive" : c.days < 30 ? "text-warning" : ""}`}>
...
function KPI({ label, value, icon, tone = "default" }: { label: string; value: number | string; icon: React.ReactNode; tone?: "default" | "success" | "warning" | "destructive" }) {
  const tones = { default: "", success: "text-success", warning: "text-warning", destructive: "text-destructive" };
  return (
