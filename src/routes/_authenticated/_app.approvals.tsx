...
export const Route = createFileRoute("/_authenticated/_app/approvals")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Gates de coleção, ficha técnica, piloto e liberação para produção.",
      },
    ],
  }),
  component: Approvals,
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Gate[]> => {
      const [cols, fichas, protos, pos] = await Promise.all([
        ...
        supabase
          .from("tech_sheets")
          .select("id, version, status, product_id, products(name, sku)")
          .order("created_at", { ascending: false })
          .limit(50),
        supabase
          .from("prototypes")
          .select("id, code, stage, product_id, products(name)")
          .order("created_at", { ascending: false })
          .limit(50),
        supabase
          .from("production_orders")
          .select("id, code, status, stage, quantity")
          .order("created_at", { ascending: false })
          .limit(50),
      ]);
      ...
      return [
        {
          key: "colecao",
          title: "1. Coleção",
          description: "Aprovação do briefing e cronograma.",
          items: (cols.data ?? [])
            .filter(
              (c) =>
                c.status === "briefing" || c.status === "desenvolvimento" || c.status === "design",
            )
            .map((c) => ({
              id: c.id,
              label: c.name,
        ...
        },
        {
          ...
          description: "Engenharia aprovada por produto.",
          items: (fichas.data ?? [])
        ...
        },
        {
          key: "piloto",
          title: "3. Piloto",
          description: "Prova aprovada antes de produzir.",
          items: (protos.data ?? [])
            .filter((p) => p.stage === "em_prova" || p.stage === "solicitado")
            .map((p) => ({
              id: p.id,
        ...
        {
          key: "producao",
          title: "4. Liberação p/ Produção",
          description: "OPs aguardando início.",
          items: (pos.data ?? [])
            .filter((o) => o.status === "aguardando" || o.stage === "cad")
            .map((o) => ({
              id: o.id,
              label: o.code,
              sub: `${o.quantity} pç`,
            ...
            (data ?? []).map((g) => (
              <div key={g.key}>
                <div className="font-semibold mb-2 flex items-center gap-2">
                  {g.title} <span className="text-xs text-muted-foreground">— {g.description}</span>
                </div>
