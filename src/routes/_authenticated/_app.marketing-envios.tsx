  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Envios de Marketing · USE MODA OS" },
      {
        ...
        content: "Controle de envios para influenciadores e imprensa.",
      },
...
async function loadAll() {
  const [{ data: shipments }, { data: collections }, { data: influencers }, { data: products }] =
    await Promise.all([
      supabase
    ...
    `,
      ...
      supabase
        .from("collections")
        .select("id, name, season, year, created_at")
        .order("created_at", { ascending: false })
        .limit(60),
...
function MarketingEnvios() {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({
      ...
      id: string;
      patch: { status?: string; shipped_at?: string; posted_at?: string };
    }) => {
  ...
  const newCollections = useMemo(() => {
    ...
    return (data?.collections ?? []).filter(
      (c) => new Date(c.created_at).getTime() > cutoff && !sent.has(c.id),
    );
  }, [data]);
  ...
  const kpi = useMemo(() => {
    ...
    const totalUplift = list.reduce(
      (s, x) => s + (Number(x.sales_after) - Number(x.sales_before)),
      0,
    );
    return {
      total: list.length,
      enviados: list.filter(
        (s) => s.status === "enviado" || s.status === "entregue" || s.status === "postado",
      ).length,
  ...
  return (
    <div className="p-6 space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
            <Send className="size-6 text-primary" /> Envios para Influencers
          </h1>
          <p className="text-sm text-muted-foreground">
            Controle das peças enviadas, performance pós-postagem e sugestões da IA por coleção.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="Uplift total"
          value={`R$ ${kpi.uplift.toLocaleString("pt-BR", { maximumFractionDigits: 0 })}`}
          icon={
            kpi.uplift >= 0 ? (
          ...
          tone={kpi.uplift >= 0 ? "success" : "destructive"}
        />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2 font-medium">
            <Sparkles className="size-4 text-primary" /> Sugestões da IA
          </div>
        ...
        {aiSuggestions.length === 0 ? (
          <div className="p-6 text-sm text-muted-foreground text-center">
            Selecione uma coleção e clique em Gerar para receber sugestões personalizadas.
          </div>
        ) : (
            ...
            {aiSuggestions.map((s, idx) => (
              <div key={idx} className="px-4 py-3 grid grid-cols-12 gap-3 items-center text-sm">
                <div className="col-span-12 md:col-span-3 font-medium">
                  {s.product_name}{" "}
                  <span className="text-xs text-muted-foreground">{s.product_sku}</span>
                </div>
      ...
      {upliftChart.length > 0 && (
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-sm font-medium mb-3 flex items-center gap-2">
            <TrendingUp className="size-4" /> Vendas antes vs depois da divulgação
          </div>
          <div className="h-64">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!isLoading && (data?.shipments.length ?? 0) === 0 && (
              <tr>
                <td colSpan={9} className="p-8 text-center text-muted-foreground">
              ...
              return (
                <tr key={s.id} className="border-t border-border hover:bg-muted/30">
                      ...
                      onChange={(e) =>
                        updateStatus.mutate({
                          ...
                          patch: {
                            ...
                            ...(e.target.value === "enviado" && !s.shipped_at
                              ? { shipped_at: new Date().toISOString().slice(0, 10) }
                              : {}),
                            ...(e.target.value === "postado" && !s.posted_at
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">
                    R${" "}
                    {Number(s.sales_before).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums">
                    R$ {Number(s.sales_after).toLocaleString("pt-BR", { maximumFractionDigits: 0 })}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums font-semibold ${uplift > 0 ? "text-emerald-600 dark:text-emerald-400" : uplift < 0 ? "text-destructive" : ""}`}
                  >
...
function NewShipmentForm({
  ...
  const filteredProducts = collection_id
    ? products.filter((p) => p.collection_id === collection_id)
    : products;
