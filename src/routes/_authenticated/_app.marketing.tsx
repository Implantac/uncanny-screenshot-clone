...
function Marketing() {
  ...
  const channels = useMemo(
    () => Array.from(new Set(rows.map((c) => c.channel).filter(Boolean) as string[])),
    [rows],
  );
  ...
  const filtered = useMemo(
    () =>
      channelFilter === "todos"
        ? periodFiltered
        : periodFiltered.filter((c) => c.channel === channelFilter),
  ...
  const roasAvg = filtered.length
    ? filtered.reduce((a, b) => a + Number(b.roas), 0) / filtered.length
    : 0;
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 animate-fade-in">
          ...
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-1">
                Performance por produto e coleção · dados de receita lidos do ERP ·{" "}
                {filtered.length} de {rows.length} campanhas
              </p>
              <p className="text-[11px] text-muted-foreground/80 mt-1">
                ⓵ O PLM mede o impacto do produto; o ERP permanece como fonte oficial financeira.
              </p>
            </div>
          </div>
              ...
              onClick={() =>
                exportToCsv(
                  ...
                  filtered.map((c) => ({
                    ...
                    receita_est: Number(c.investment) * Number(c.roas),
                  })),
                  [
                    ...
                    { key: "roas", label: "ROAS" },
                    { key: "receita_est", label: "Receita estimada" },
        ...
        <div className="relative mt-6 flex items-center gap-2 flex-wrap pt-5 border-t border-border/50">
          ...
          {(
            ...
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs font-medium border transition-all ${periodFilter === p.k ? "bg-primary text-primary-foreground border-primary shadow-[var(--shadow-glow)]" : "border-border text-muted-foreground hover:bg-muted hover:text-foreground"}`}
            >
              {p.label}
            </button>
          ))}
          {channels.length > 0 && (
            <span className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground ml-3 mr-1 font-semibold">
              Canal
            </span>
          )}
          {channels.length > 0 && (
            <>
              <button
                ...
                className={`px-3 py-1 rounded-md text-xs font-medium border transition-all ${channelFilter === "todos" ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted hover:text-foreground"}`}
              >
                Todos
              </button>
              {channels.map((ch) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-xs font-medium border transition-all ${channelFilter === ch ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:bg-muted hover:text-foreground"}`}
                >
                  {ch}
                </button>
              ))}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          trend={{ dir: lucro >= 0 ? "up" : "down", pct: Math.abs(margemPct) }}
          hint={`Margem ${margemPct.toFixed(0)}%`}
        />
        <StatCard
          label="ROAS médio"
          value={`${roasAvg.toFixed(2)}x`}
          icon={Target}
          color={roasAvg >= 3 ? "#10b981" : roasAvg >= 2 ? "#f59e0b" : "#ef4444"}
          hint={roasAvg >= 3 ? "Excelente" : roasAvg >= 2 ? "Saudável" : "Atenção"}
          badge={`${ativas} ativas`}
        />
      </div>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="brief">Brief Studio</TabsTrigger>
        ...
        <TabsContent value="campaigns">
          {isLoading ? (
            ...
            <div className="glass rounded-xl p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                {rows.length === 0
                  ...
                  : "Nenhuma campanha no filtro selecionado."}
              </p>
              {rows.length === 0 && (
                <Button
                  onClick={() => {
            ...
            <div className="glass rounded-xl overflow-hidden">
              <div className="p-5 border-b border-border text-sm font-semibold inline-flex items-center gap-2">
                <Calendar className="size-4" /> Campanhas
              </div>
                      ...
                      return (
                        <tr key={c.id} className="border-t border-border hover:bg-muted/30">
                          ...
                          <td className="px-5 py-3 text-right tabular-nums">
                            {brl(Number(c.investment))}
                          </td>
                          <td className="px-5 py-3 text-right tabular-nums font-medium">
                            {Number(c.roas) > 0 ? `${Number(c.roas).toFixed(1)}x` : "—"}
                          </td>
                          <td className="px-5 py-3">
                            <span
                              className={`px-2 py-0.5 rounded text-xs ${STATUS_STYLE[c.status]}`}
                            >
                          ...
                          <td className="px-5 py-3 text-right">
                            {mine && (
                              <div className="flex justify-end gap-1">
                                ...
                                <button
                                  onClick={() =>
                                    confirm("Remover esta campanha?") && deleteMut.mutate(c.id)
                                  }
...
function CampaignDialog({
  ...
  useEffect(() => {
    if (open && editing) {
      ...
      setInvestment(String(e.investment));
      setRoas(String(e.roas));
      setStatus(e.status);
      setNotes(e.notes || "");
      setCostShoot(String(e.cost_shoot ?? 0));
      setCostPhotos(String(e.cost_photos ?? 0));
      setCostTraffic(String(e.cost_traffic ?? 0));
      setRevenue(String(e.revenue ?? 0));
    } else if (open) {
      setName(prefillName || "");
      setChannel("");
      setStartDate("");
          ...
          <div className="pt-2 border-t border-border space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">
                Custo detalhado · por peça/coleção
              </div>
              {(() => {
                const totalCost =
                  Number(costShoot) + Number(costPhotos) + Number(costTraffic) + Number(investment);
                ...
                return (
                  <div className="text-[10px] tabular-nums text-muted-foreground">
                    Lucro:{" "}
                    <span className={lucro >= 0 ? "text-emerald-500" : "text-destructive"}>
                      R$ {lucro.toFixed(0)}
                    </span>{" "}
                    · ROI {roi.toFixed(0)}%
                  </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                <Label>Ensaio (R$)</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={costShoot}
                  onChange={(e) => setCostShoot(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label>Fotos (R$)</Label>
                <Input
                  ...
                  step="0.01"
                  value={costPhotos}
                  onChange={(e) => setCostPhotos(e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label>Tráfego (R$)</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={costTraffic}
                  onChange={(e) => setCostTraffic(e.target.value)}
                />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Vindo do ERP ou registrado manualmente"
              />
            </div>
          </div>
          <div className="space-y-2">
...
function StatCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-card p-5 group hover:border-primary/50 hover:shadow-[var(--shadow-glow)] transition-all">
      ...
      <div
        className="absolute -right-8 -top-8 size-28 rounded-full opacity-[0.08] blur-2xl group-hover:opacity-20 transition-opacity"
        style={{ background: c }}
      />
      <div className="relative flex items-start justify-between gap-2">
        ...
        {trend && (
          <span
            className={`inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded ${trend.dir === "up" ? "bg-emerald-500/15 text-emerald-400" : "bg-rose-500/15 text-rose-400"}`}
          >
            {trend.dir === "up" ? (
      ...
      <div
        className="relative text-3xl font-bold mt-3 tabular-nums tracking-tight"
        style={{ color: c }}
      >
        {value}
      </div>
...
function InsightsBar({
  ...
  const insights = useMemo(() => {
    ...
    const worst = [...rows]
      .filter((c) => Number(c.roas) > 0)
      .sort((a, b) => Number(a.roas) - Number(b.roas))[0];
    if (best)
      list.push({
        icon: Zap,
        tone: "text-emerald-400 bg-emerald-500/10",
        text: `Melhor ROAS: ${best.name} (${Number(best.roas).toFixed(1)}x)`,
      });
    if (worst && worst.id !== best?.id && Number(worst.roas) < 2)
      list.push({
        icon: AlertTriangle,
        tone: "text-amber-400 bg-amber-500/10",
        text: `Revisar: ${worst.name} (${Number(worst.roas).toFixed(1)}x)`,
      });
    if (roasAvg >= 3)
      list.push({
        icon: Award,
        tone: "text-emerald-400 bg-emerald-500/10",
        text: `Carteira performando acima do mercado (${roasAvg.toFixed(1)}x)`,
      });
    else if (roasAvg < 2 && invTotal > 0)
      list.push({
        icon: AlertTriangle,
        tone: "text-rose-400 bg-rose-500/10",
        text: `ROAS médio abaixo de 2x — otimizar criativos`,
      });
    const lucro = receitaEst - invTotal;
    if (lucro > 0 && invTotal > 0)
      list.push({
        icon: TrendingUp,
        tone: "text-sky-400 bg-sky-500/10",
        text: `Margem projetada de ${((lucro / invTotal) * 100).toFixed(0)}% sobre investimento`,
      });
  ...
  return (
    <div className="glass rounded-xl p-4">
      <div className="text-xs font-semibold inline-flex items-center gap-1.5 mb-3 text-muted-foreground">
...
function ChartsSection({ rows }: { rows: Campaign[] }) {
  const byChannel = useMemo(() => {
    const m = new Map<
      string,
      { channel: string; investimento: number; receita: number; n: number }
    >();
    rows.forEach((c) => {
  ...
  const byMonth = useMemo(() => {
    ...
    return Array.from(m.values())
      ...
      .map((d) => ({
        ...
        mesLabel: new Date(d.mes + "-01").toLocaleDateString("pt-BR", {
          month: "short",
          year: "2-digit",
        }),
      }));
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="glass rounded-xl p-5 lg:col-span-2">
        <div className="flex items-start justify-between mb-4 gap-4 flex-wrap">
          <div>
            ...
            <div className="text-[11px] text-muted-foreground mt-0.5">
              Barras: investimento e receita · Linha: ROAS · Meta: {ROAS_GOAL}x
            </div>
          </div>
          <div className="flex items-center gap-3 text-[11px]">
            ...
            <span className="inline-flex items-center gap-1.5">
              <span className="h-0.5 w-3 bg-[var(--primary)]" />
              ROAS
            </span>
          </div>
        </div>
        {byMonth.length === 0 ? (
          <div className="text-sm text-muted-foreground text-center py-16">
            Sem datas de início informadas.
          </div>
        ) : (
            ...
            <ComposedChart data={byMonth} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(v: number, name: string) =>
                  name === "ROAS" ? [`${v.toFixed(2)}x`, name] : [brl(v), name]
                }
              />
              <ReferenceLine
                yAxisId="roas"
      ...
      <div className="glass rounded-xl p-5 flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-1.5 mt-2">
          {byStatus.map((s) => (
            <div
              key={s.status}
              className="flex items-center justify-between text-[11px] px-2 py-1 rounded bg-muted/40"
            >
              <span className="inline-flex items-center gap-1.5">
                <span
      ...
      <div className="glass rounded-xl p-5">
        <div className="text-sm font-semibold mb-1 inline-flex items-center gap-1.5">
              ...
              return (
                <div key={i} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      className={`tabular-nums font-semibold ${positive ? "text-emerald-400" : "text-rose-400"}`}
                    >
                      {positive ? "+" : ""}
                      {brl(c.lucro)}
                    </span>
                  ...
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      className={`h-full rounded-full ${positive ? "bg-gradient-to-r from-emerald-500 to-emerald-300" : "bg-gradient-to-r from-rose-500 to-rose-300"}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <div className="flex items-center justify-between text-[10px] text-muted-foreground tabular-nums">
                    <span>
                      {brl(c.investimento)} → {brl(c.receita)}
                    </span>
...
function AdvancedSection({ rows }: { rows: Campaign[] }) {
  ...
  const pipeline = useMemo(() => {
    const counts: Record<CStatus, { count: number; invest: number }> = {
      programada: { count: 0, invest: 0 },
      ativa: { count: 0, invest: 0 },
      pausada: { count: 0, invest: 0 },
      concluida: { count: 0, invest: 0 },
    };
    rows.forEach((c) => {
      counts[c.status].count += 1;
      counts[c.status].invest += Number(c.investment);
    });
      ...
      <div className="glass rounded-xl p-5 lg:col-span-2">
        <div className="flex items-center justify-between mb-1">
          <div className="text-sm font-semibold inline-flex items-center gap-1.5">
            ...
            Matriz BCG · ROAS × Investimento
          </div>
          <div className="text-[11px] text-muted-foreground">Bolha = receita estimada</div>
        </div>
        <div className="text-[11px] text-muted-foreground mb-2">
          Superior direito: <span className="text-emerald-400">estrelas</span> · Inferior direito:{" "}
          <span className="text-rose-400">drenando caixa</span>
        </div>
        <ResponsiveContainer width="100%" height={320}>
          <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
            ...
            <ReferenceLine
              ...
              stroke="#ef4444"
              strokeDasharray="4 4"
              label={{ value: "Breakeven", fontSize: 10, fill: "#ef4444", position: "right" }}
            />
            {medInv > 0 && (
              <ReferenceLine x={medInv} stroke="var(--muted-foreground)" strokeDasharray="4 4" />
            )}
            <Tooltip
              contentStyle={tooltipStyle}
                ...
                return (
                  <div className="rounded-lg border border-border bg-card px-3 py-2 text-xs shadow-xl">
                    ...
                    <div className="text-muted-foreground">
                      Investimento: <span className="text-foreground tabular-nums">{brl(d.x)}</span>
                    </div>
                    <div className="text-muted-foreground">
                      ROAS: <span className="text-foreground tabular-nums">{d.y.toFixed(2)}x</span>
                    </div>
                    <div className="text-muted-foreground">
                      Receita: <span className="text-emerald-400 tabular-nums">{brl(d.z)}</span>
                    </div>
                    <div className="text-muted-foreground">
                      Lucro:{" "}
                      <span
                        className={`tabular-nums ${lucro >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {brl(lucro)}
                      </span>
                    </div>
            ...
            <Scatter data={matrix}>
              {matrix.map((d, i) => (
                <Cell
                  key={i}
                  fill={STATUS_COLORS[d.status as CStatus] ?? "#3b82f6"}
                  fillOpacity={0.75}
                />
              ))}
            </Scatter>
            ...
            <div key={p.key} className="space-y-1">
              ...
              <div className="h-2 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{ width: `${(p.count / maxPipeline) * 100}%`, background: p.fill }}
                />
              </div>
              <div className="text-[10px] text-muted-foreground tabular-nums">
                {brl(p.invest)} alocado
              </div>
            </div>
          ))}
        </div>
        ...
        {channelEff.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {channelEff.map((c) => {
              ...
              const labelTone =
                c.roas >= 3 ? "text-emerald-400" : c.roas >= 2 ? "text-amber-400" : "text-rose-400";
              return (
                <div
                  key={c.channel}
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`text-xs font-semibold tabular-nums ${labelTone}`}>
                      {c.roas.toFixed(2)}x
                    </div>
                  </div>
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                  ...
                  <div className="text-[10px] tabular-nums pt-1 border-t border-border/50 flex justify-between">
                    ...
                    <span className={c.lucro >= 0 ? "text-emerald-400" : "text-rose-400"}>
                      {c.lucro >= 0 ? "+" : ""}
                      {brl(c.lucro)}
                    </span>
                  </div>
...
function channelSuggestion(category: string | null) {
  ...
  if (c.includes("fem") || c.includes("vest") || c.includes("saia"))
...
function ReadyToLaunchCard({
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-4 space-y-3">
      ...
      <p className="text-sm">
        <span className="font-semibold">{pending.length}</span> protótipo
        {pending.length > 1 ? "s" : ""} aprovado{pending.length > 1 ? "s" : ""} sem campanha
        vinculada — sugestão de canal por categoria abaixo.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
        {pending.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className="rounded-lg border border-border bg-background/60 p-3 flex items-center gap-3"
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-muted-foreground truncate">
                  {prod.category ?? "—"} · <span className="text-primary">{sug}</span>
                </div>
              </div>
              <Button
                size="sm"
                variant="outline"
                onClick={() => onCreate(`Lançamento ${prod.name}`)}
