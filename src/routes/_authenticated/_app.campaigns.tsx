...
async function load(): Promise<Camp[]> {
  const [{ data }, { data: erpRows }] = await Promise.all([
    ...
    supabase
      ...
      .select("campaign_code, total_value")
...
function Campaigns() {
  ...
  const byChannel = useMemo(() => {
    ...
    return Array.from(m.values())
      .map((c) => ({ ...c, roas: c.inv > 0 ? c.rev / c.inv : 0 }))
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Investimento, ROAS, receita atribuída e performance por canal.
        </p>
      </header>
          ...
          <div className="divide-y divide-border">
            {byChannel.map((c) => (
              <div key={c.ch} className="px-4 py-3 grid grid-cols-12 gap-3 items-center text-sm">
                ...
                <div
                  className={`col-span-6 md:col-span-2 text-right font-semibold ${c.roas >= 3 ? "text-success" : c.roas >= 1 ? "text-warning" : "text-destructive"}`}
                >
                  {c.roas.toFixed(2)}x
                </div>
              </div>
            ...
            <tbody>
              ...
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td
                    className={`px-3 py-2 text-right ${r.margin >= 0 ? "text-success" : "text-destructive"}`}
                  >
                    {fmt(r.margin)}
                  </td>
                  <td
