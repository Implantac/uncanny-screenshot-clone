...
export const Route = createFileRoute("/_authenticated/_app/variantes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Cores, tamanhos e SKUs por produto. Base para grade de produção.",
      },
    ],
  }),
...
function VariantsPage() {
  ...
  const addColor = useMutation({
    mutationFn: async (v: { name: string; hex: string }) => {
      const { error } = await (supabase as any).from("product_color_options").insert({
        ...
        name: v.name,
        hex: v.hex || null,
        position: (colors.data?.length ?? 0) + 1,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["variant-colors", productId] });
  ...
  const addSize = useMutation({
    mutationFn: async (v: { label: string }) => {
      const { error } = await (supabase as any).from("product_size_options").insert({
        ...
        product_id: productId,
        label: v.label,
        position: (sizes.data?.length ?? 0) + 1,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["variant-sizes", productId] });
  ...
  const delColor = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from("product_color_options").delete().eq("id", id);
      if (error) throw error;
    },
  ...
  const delSize = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from("product_size_options").delete().eq("id", id);
      if (error) throw error;
    },
  ...
  const generate = useMutation({
    mutationFn: async () => {
      ...
      const existing = new Set(
        (variants.data ?? []).map((v) => `${v.color_id ?? ""}|${v.size_id ?? ""}`),
      );
      const rows: any[] = [];
      ...
      const pairs: Array<{ c: Color | null; s: Size | null }> =
        cs.length && ss.length
          ? cs.flatMap((c) => ss.map((s) => ({ c, s })))
          : cs.length
            ? cs.map((c) => ({ c, s: null as any }))
            : ss.length
              ? ss.map((s) => ({ c: null as any, s }))
              : [];
    ...
    onSuccess: (n) => {
      ...
      toast.success(`${n} SKU(s) gerado(s)`);
    },
    onError: (e: any) => toast.error(e.message),
  });
      ...
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Cores, tamanhos e SKUs gerados por combinação.
          </p>
        </div>
      ...
      {!productId ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ColorPanel
            ...
            onAdd={(v) => addColor.mutate(v)}
            onDelete={(id) => delColor.mutate(id)}
          />
          <SizePanel
            sizes={sizes.data ?? []}
            loading={sizes.isLoading}
            onAdd={(v) => addSize.mutate(v)}
            onDelete={(id) => delSize.mutate(id)}
          />
                  ...
                  {variantRows.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-muted-foreground">
                        Sem variantes ainda. Adicione cores e tamanhos e clique em Gerar.
                      </td>
                    </tr>
                  ) : (
                    variantRows.map((v) => (
...
function ColorPanel({
  ...
  colors: Color[];
  loading: boolean;
  onAdd: (v: { name: string; hex: string }) => void;
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="space-y-1">
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">Nenhuma cor.</p>
          ) : (
            colors.map((c) => (
              <div
                key={c.id}
...
function SizePanel({
  ...
  return (
    <div className="rounded-xl border border-border bg-card">
      <div className="p-4 border-b border-border">
        <h2 className="font-medium">Tamanhos</h2>
      </div>
      <div className="p-4 space-y-3">
        <div className="flex gap-2">
          <Input
        ...
        <div className="flex flex-wrap gap-2">
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">Nenhum tamanho.</p>
          ) : (
            sizes.map((s) => (
              <Badge key={s.id} variant="secondary" className="gap-1">
                {s.label}
                <button onClick={() => onDelete(s.id)} className="ml-1 hover:text-destructive">
