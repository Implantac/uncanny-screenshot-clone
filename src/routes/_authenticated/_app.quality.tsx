...
import { MaterialSourcingRiskPanel } from "@/components/material-sourcing-risk-panel-lazy";
...
export const Route = createFileRoute("/_authenticated/_app/quality")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Inspeção de lotes, defeitos por fornecedor e SLA de entrega.",
      },
    ],
  }),
  component: Quality,
    ...
    queryFn: async () => {
      const [{ data: suppliers }, { data: orders }, { data: services }] = await Promise.all([
        ...
        supabase
          ...
          .select("id, code, supplier_id, status, stage, due_date, quantity, created_at"),
        supabase
          .from("service_orders")
          .select(
            "id, supplier_id, status, sent_at, due_at, received_at, kind, quantity, qty_received",
          ),
      ]);
      ...
      const slaAgg = new Map<
        string,
        { total: number; onTime: number; late: number; defects: number }
      >();
      (services ?? []).forEach((so) => {
        ...
        if (
          so.qty_received != null &&
          so.quantity != null &&
          Number(so.qty_received) < Number(so.quantity)
        ) {
      ...
      const supplierScores = Array.from(slaAgg.entries())
        .map(([id, a]) => {
      ...
      const inspections = (orders ?? [])
        .filter((o) =>
          ["controle_qualidade", "embalagem", "expedicao"].includes(String(o.stage ?? "")),
        )
        .slice(0, 30);

      // SLA crítico
      const overdueOps = (orders ?? []).filter(
        (o) => o.due_date && new Date(o.due_date).getTime() < now && o.status !== "concluida",
      ).length;

      const avgScore = supplierScores.length
        ? Math.round(supplierScores.reduce((a, s) => a + s.score, 0) / supplierScores.length)
        : 0;
      const avgOtd = supplierScores.length
        ? supplierScores.reduce((a, s) => a + s.otd, 0) / supplierScores.length
        : 0;
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl text-cyan-600">{data?.avgScore ?? 0}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>OTD médio</CardDescription>
            <CardTitle className="text-2xl">{(data?.avgOtd ?? 0).toFixed(1)}%</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>OPs atrasadas</CardDescription>
            <CardTitle className="text-2xl text-red-600">{data?.overdueOps ?? 0}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
      ...
      <Tabs defaultValue="suppliers">
        ...
        <TabsContent value="suppliers">
          <Card>
            <CardHeader>
              ...
              <CardDescription>OTD, taxa de defeito e score composto.</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                  ...
                  <table className="w-full text-sm">
                    <thead className="text-left text-xs text-muted-foreground border-b">
                      <tr>
                        ...
                        <th className="text-right">Defeito</th>
        ...
        <TabsContent value="inspections">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" /> Inspeções pendentes
              </CardTitle>
              <CardDescription>
                Ordens em controle de qualidade, embalagem ou expedição.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {data?.inspections.map((o) => (
                  <Link
                    key={o.id}
                    to="/pcp-kanban"
                    className="block border rounded-lg p-3 hover:bg-muted"
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="font-medium font-mono text-sm">{o.code}</div>
                        <div className="text-xs text-muted-foreground">
                          Estágio: {o.stage} · {o.quantity} unid.
                        </div>
                      </div>
        ...
        <TabsContent value="capa" className="space-y-4">
          <RootCausePanel windowDays={60} minOccurrences={3} />
