...
export const Route = createFileRoute("/_authenticated/_app/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Pulso do desenvolvimento de produto e produção em tempo real.",
      },
    ],
  }),
...
function useDashboard() {
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      const productsInDev = pr.filter(
        (r: any) => !r.status || /dev|brief|model|piloto|prot/i.test(r.status),
      ).length;
      const piecesInProd = p
      ...
      const productsWithApprovedSheet = new Set(
        ts
          .filter((t: any) => t.status === "aprovada" && t.product_id)
          .map((t: any) => t.product_id as string),
      );
      const productsWithoutSheet = pr.filter(
        (r: any) =>
          r.id &&
          !productsWithApprovedSheet.has(r.id) &&
          (!r.status || /dev|brief|model|piloto|prot|aprov/i.test(r.status)),
      );
      const pendingPilots = pt.filter(
        (r: any) => r.stage && /solicit|em_prova|ajuste|pend/i.test(r.stage),
      );
      const STUCK_DAYS = 5;
      const now = Date.now();
      ...
      const done = p.reduce(
        (a, b) => a + Math.round((b.quantity ?? 0) * ((b.progress ?? 0) / 100)),
        0,
      );
      const productionData = [
        ...
        {
          d: "Concluído",
          v: p.filter((r) => r.status === "concluida").reduce((a, b) => a + (b.quantity ?? 0), 0),
        },
      ];
      ...
      type FeedItem = {
        ts: number;
        kind: "produto" | "coleção" | "produção" | "ficha" | "protótipo";
        title: string;
        meta?: string;
      };
      const feed: FeedItem[] = [];
      pr.slice(0, 8).forEach(
        (r: any) =>
          r.created_at &&
          feed.push({
            ts: new Date(r.created_at).getTime(),
            kind: "produto",
            title: `Produto ${r.name} criado`,
      ...
      p.slice(0, 8).forEach(
        (r: any) =>
          ...
          feed.push({
            ts: new Date(r.created_at).getTime(),
            kind: "produção",
            title: `OP ${r.code ?? ""} · ${r.quantity ?? 0} pç`,
            meta: r.status,
          }),
      );
      pt.slice(0, 8).forEach(
        (r: any) =>
          r.created_at &&
          feed.push({
            ts: new Date(r.created_at).getTime(),
            kind: "protótipo",
            title: `Protótipo ${r.code ?? ""}`,
            meta: r.stage,
          }),
      );
      ts.slice(0, 8).forEach(
        (r: any) =>
          r.created_at &&
          feed.push({
            ts: new Date(r.created_at).getTime(),
      ...
      const hotCollection =
        [...activeCols].sort((a: any, b: any) => (b.progress ?? 0) - (a.progress ?? 0))[0] ??
        c[0] ??
        null;

      // === Marketing ROI consolidado ===
      const activeCamps = cmp.filter((r: any) => r.status === "ativa" || r.status === "active");
      const baseCamps = (activeCamps.length ? activeCamps : cmp).filter(
        (r: any) => Number(r.roas) > 0,
      );
      const invTotal = baseCamps.reduce((a: number, b: any) => a + Number(b.investment ?? 0), 0);
      const recTotal = baseCamps.reduce(
        (a: number, b: any) => a + Number(b.investment ?? 0) * Number(b.roas ?? 0),
        0,
      );
      const roasAvg = baseCamps.length
        ? baseCamps.reduce((a: number, b: any) => a + Number(b.roas ?? 0), 0) / baseCamps.length
        : 0;
      ...
      const marketing = {
        ...
        worst:
          worstCampaign && worstCampaign !== bestCampaign
            ? { name: worstCampaign.name, roas: Number(worstCampaign.roas) }
            : null,
      };
  ...
  const kpis = [
    {
      ...
      value: k ? String(k.activeCollections) : "—",
      icon: Layers,
      color: "text-primary",
    },
    {
      label: "Produtos em desenvolvimento",
      value: k ? String(k.productsInDev) : "—",
      icon: Sparkles,
      color: "text-info",
    },
    {
      label: "Peças em produção",
      value: k ? k.piecesInProd.toLocaleString("pt-BR") : "—",
      icon: Factory,
      color: "text-success",
    },
    {
      label: "Protótipos em aberto",
      value: k ? String(k.protosOpen) : "—",
      icon: Scissors,
      color: "text-warning",
    },
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            ...
            <span className="ml-2 inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground uppercase tracking-wider">
              Financeiro no ERP
            </span>
          </p>
        </div>
      ...
      <Tabs defaultValue="resumo" className="space-y-4">
        ...
        <TabsContent value="resumo" className="space-y-4 mt-2">
          ...
          {/* Coleção em destaque + Marketing ROI — lazy: só monta quando rolam até aqui (evita render/queries extras na 1ª carga). */}
          <LazyReveal minHeight={220} placeholderClass="bg-muted/20 rounded-xl animate-pulse">
