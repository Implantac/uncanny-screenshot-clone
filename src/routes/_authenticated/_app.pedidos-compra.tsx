      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Cotações e pedidos. Aprovados geram contas a pagar; recebidos lançam entrada no estoque.
          </p>
        </div>
        <Button
          onClick={() => {
              ...
              pos.map((p) => (
                <tr key={p.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    {p.supplier_id ? (supMap.get(p.supplier_id) ?? "—") : "—"}
                  </td>
                  <td className="px-3 py-2">
                    <Select
                      value={p.status}
                  ...
                  <td className="px-3 py-2 text-xs text-muted-foreground">
                    {p.expected_date ? new Date(p.expected_date).toLocaleDateString("pt-BR") : "—"}
                  </td>
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="flex justify-end gap-1">
                      ...
                      <button
                        onClick={() => confirm("Remover este pedido?") && delMut.mutate(p.id)}
  ...
  useEffect(() => {
    if (open && editing) {
      setF({
        ...
        total_value: Number(editing.total_value),
        notes: editing.notes ?? "",
      });
    } else if (open) {
      setF({
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        total_value: totalCalc || f.total_value,
        notes: f.notes || null,
      };
      let poId: string;
      if (editing) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Itens, fornecedor e prazo. O total é calculado automaticamente.
          </DialogDescription>
        </DialogHeader>
        <form
          onSubmit={(e) => {
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                onClick={() =>
                  setItems([
                    ...items,
                    { inventory_item_id: null, description: "", quantity: 1, unit_price: 0 },
                  ])
                }
              >
            ...
            {items.map((it, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_70px_90px_30px] gap-2 items-center">
                <Select
                  value={it.inventory_item_id ?? "free"}
                  onValueChange={(v) => {
                    const next = [...items];
                ...
                >
                  <SelectTrigger className="h-9">
                    <SelectValue placeholder="Item / descrição livre" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    const n = [...items];
                    n[idx].unit_price = Number(e.target.value);
                    setItems(n);
                  }}
                  placeholder="R$"
                />
                <button
                  ...
                  className="size-8 grid place-items-center rounded hover:bg-destructive/20 text-destructive"
                >
                  <Trash2 className="size-3.5" />
                </button>
            ...
            <div className="flex justify-end pt-2 border-t border-border">
              <Badge variant="outline">
                Total: R$ {totalCalc.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
              </Badge>
            </div>
          </div>
