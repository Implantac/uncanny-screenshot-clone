...
async function load(): Promise<Row[]> {
  ...
  const [{ data: products }, { data: sales }] = await Promise.all([
    supabase
      .from("products")
      .select("id, sku, name, cost_price, sell_price, collections(name)")
      .limit(1000),
...
function MargemPage() {
  ...
  const filtered = useMemo(
    () => (filter === "all" ? rows : rows.filter((r) => r.abc === filter)),
    [rows, filter],
  );
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Margem real por SKU e variação de custo (teórico × real) por OP.
        </p>
      </header>
                  ...
                  {filtered.slice(0, 200).map((r) => (
                    <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                      <td className="px-3 py-2">
                        <span
                          className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold ${r.abc === "A" ? "bg-success/15 text-success" : r.abc === "B" ? "bg-warning/15 text-warning" : "bg-muted text-muted-foreground"}`}
                        >
                          {r.abc}
                        </span>
                      </td>
                      ...
                      <td
                        className={`px-3 py-2 text-right font-semibold ${r.margin >= 0 ? "text-success" : "text-destructive"}`}
                      >
                        {fmt(r.margin)}
                      </td>
                      <td
                        className={`px-3 py-2 text-right ${r.marginPct >= 40 ? "text-success" : r.marginPct >= 20 ? "text-warning" : "text-destructive"}`}
                      >
