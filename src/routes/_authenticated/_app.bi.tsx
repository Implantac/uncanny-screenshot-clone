...
function BI() {
  ...
  const persona = isAdmin
    ? "admin"
    : sectors.includes("pcp")
      ? "pcp"
      : sectors.includes("desenvolvimento")
        ? "dev"
        : sectors.includes("marketing")
          ? "marketing"
          : "admin";
  const defaultTab =
    persona === "pcp"
  ...
  const personaInfo: Record<string, { title: string; subtitle: string }> = {
    ...
    pcp: {
      title: "Visão PCP",
      subtitle: "Foco em ordens, etapas e gargalos. Tab Produção aberta por padrão.",
    },
    dev: {
      title: "Visão Desenvolvimento",
      subtitle: "Pipeline de produtos e protótipos. Tab Desenvolvimento aberta.",
    },
    marketing: {
      title: "Visão Marketing",
      subtitle: "Campanhas, canais e envios a influenciadores.",
    },
  };
  ...
  const ordersQ = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("b2b_orders").select("status,total_value,order_date")).data ?? [],
  });
  ...
  const finQ = useQuery({
    queryKey: ["bi", "fin"],
    queryFn: async () =>
      (await supabase.from("financial_accounts").select("type,value,status")).data ?? [],
  });
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select("quantity,status,stage,products(name)")
          .limit(200)
      ).data ?? [],
  });
  ...
  const qualityQ = useQuery({
    queryKey: ["bi", "quality"],
    queryFn: async () =>
      (await supabase.from("quality_inspections").select("result,inspection_type")).data ?? [],
  });
  ...
  const produtosAtivos = products.filter(
    (p) => p.status === "aprovado" || p.status === "producao",
  ).length;
  const aReceber = fin
  ...
  const trend = useMemo(() => {
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const v = orders
        .filter((o) => (o.order_date ?? "").slice(0, 10) === key)
  ...
  const kpis = [
    ...
    {
      l: "Produtos ativos",
      v: produtosAtivos.toString(),
      d: `${products.length} no total`,
      up: true,
    },
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpis.map((k) => (
          <div key={k.l} className="glass rounded-xl p-5">
            ...
            <div
              className={`text-xs mt-0.5 inline-flex items-center gap-0.5 ${k.up ? "text-success" : "text-destructive"}`}
            >
      ...
      <div className="glass rounded-xl p-3 flex items-center gap-2 border-l-2 border-primary">
        ...
        <span className="text-sm">
          <span className="font-medium">{personaInfo[persona].title}.</span>{" "}
          <span className="text-muted-foreground">Tab inicial selecionada conforme seu setor.</span>
        </span>
      </div>
        ...
        <TabsContent value="dev" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <BarPanel
              ...
              subtitle="Pipeline de desenvolvimento"
              data={devByStatus}
              empty="Sem produtos"
            />
            <BarPanel
              title="Protótipos por etapa"
              subtitle="Onde está cada protótipo"
              data={protoByStage}
              empty="Sem protótipos"
            />
          </div>
        </TabsContent>
        ...
        <TabsContent value="qualidade" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <BarPanel
              ...
              subtitle="Aprovadas / reprovadas / pendentes"
              data={qualityByResult}
              empty="Sem inspeções"
            />
            <div className="glass rounded-xl p-5">
              <div className="text-sm font-semibold">Resumo</div>
              <div className="text-xs text-muted-foreground mb-4">
                Total de inspeções: {quality.length}
              </div>
              <ul className="space-y-2 text-sm">
                {qualityByResult.map((q) => (
                  <li key={q.name} className="flex justify-between">
                    <span className="capitalize">{q.name}</span>
                    <span className="tabular-nums text-muted-foreground">{q.v}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <TabsContent value="marketing" className="space-y-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="glass rounded-xl p-5">
              ...
              <div className="text-2xl font-semibold mt-1 tabular-nums">{mktKpis.campaigns}</div>
            </div>
            <div className="glass rounded-xl p-5">
              <div className="text-xs text-muted-foreground">Orçamento total</div>
              <div className="text-2xl font-semibold mt-1 tabular-nums">{BRL(mktKpis.budget)}</div>
            </div>
            <div className="glass rounded-xl p-5">
              <div className="text-xs text-muted-foreground">Envios influenciadores</div>
          ...
          <BarPanel
            ...
            subtitle="Orçamento somado por canal"
            data={mktByChannel}
            empty="Sem campanhas"
          />
        </TabsContent>
