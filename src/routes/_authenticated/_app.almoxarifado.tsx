...
import { InventoryScrapsPanel } from "@/components/inventory-scraps-panel-lazy";
...
function Almoxarifado() {
  ...
  const reposicao = useMemo(() => {
    return items
      ...
      .sort(
        (a, b) =>
          (a.urgencia === "alta" ? -1 : 1) - (b.urgencia === "alta" ? -1 : 1) ||
          ...
          {byCat.map((b) => (
            <div key={b.cat} className="rounded-xl border border-border bg-card/50 p-4">
              ...
              <div className="text-xs text-muted-foreground">
                saldo {b.saldo.toLocaleString("pt-BR")}
              </div>
            </div>
          ))}
        </div>
            ...
            {reposicao.map((r) => (
              ...
              >
                ...
                <div className="text-right">
                  <div
                    className={`text-sm font-semibold tabular-nums ${r.urgencia === "alta" ? "text-destructive" : "text-amber-500"}`}
                  >
                    +{r.sugerido} {r.unit}
                  </div>
                  <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                    {r.urgencia === "alta" ? "Urgente" : "Planejar"}
                  </div>
                </div>
              </div>
            ))}
...
function ItemDialog({
  ...
  useEffect(() => {
    if (open && editing) {
      ...
      setMinimum(String(e.minimum));
      setMaximum(String(e.maximum || 0));
      setNotes(e.notes || "");
      setPhotoUrl(e.image_url || "");
      setSupplierColor(e.color_supplier || "");
      setInternalColor(e.color_internal || "");
      setTechSheetPdfUrl(e.tech_sheet_pdf_url || "");
    } else if (open) {
      setSku("");
      setName("");
      setCategory("tecido");
            ...
            <div className="grid grid-cols-[120px_1fr] gap-3 items-start">
              {photoUrl ? (
                <img
                  src={photoUrl}
                  alt={name || "Material"}
                  className="size-[120px] rounded-lg border border-border object-cover"
                />
              ) : (
