  ...
  return (
    <Tabs value={tab} onValueChange={setTab} className="space-y-4">
      ...
      <TabsContent value="carry-over" className="space-y-4">
        <CarryOverPanel collectionId={collectionId} collectionName={current.collection.name} />
      </TabsContent>
...
function CollectionTimelineTab({ collectionId }: { collectionId: string }) {
  ...
  const { data: milestones, isLoading } = useQuery({
    ...
    queryFn: () => list({ data: { collectionId } }),
  });

  const initMut = useMutation({
    mutationFn: () => init({ data: { collectionId } }),
    onSuccess: () => {
      ...
      <div className="rounded-xl border border-border bg-card p-8 text-center space-y-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Crie as 7 etapas (briefing → lançamento) com SLAs sugeridos.
          </p>
        </div>
