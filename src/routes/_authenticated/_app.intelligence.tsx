...
import { intelFilter } from "@/lib/intel-filter";
...
function IntelligencePage() {
  ...
  const setTab = (v: string) =>
    navigate({
      search: (p: typeof search) => ({ ...p, tab: v as typeof search.tab }),
      replace: true,
    });
  const setQ = (v: string) =>
    ...
    <div className="space-y-6">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground max-w-3xl">
          Plataforma única que responde "o que produzir, para quem vender e onde investir" —
          combinando estoque, vendas, produção, marketing e influência em tempo real.
        </p>
      </header>
      ...
      <Tabs value={search.tab} onValueChange={setTab}>
        ...
        <TabsContent value="dev">
          <DevelopmentBoard
            prototypes={intelFilter(protoQ.data ?? [], search.q, ["code", "notes"])}
          />
        </TabsContent>
        ...
        <TabsContent value="geo" className="space-y-6">
          ...
          />
          <Attribution
            campaigns={intelFilter(mktQ.data ?? [], search.q, ["name", "channel"])}
            b2b={b2bQ.data ?? []}
            sales={salesQ.data ?? []}
          />
        </TabsContent>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Cálculo: max(0, estoque máx − estoque − em produção) · cobertura em dias
            </CardDescription>
          </div>
          ...
          <CardContent className="space-y-3">
            {suggestions.map((s) => (
              <div key={s.id} data-testid="intel-item" className="rounded-lg border p-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {s.sku} · Vend {s.sold30}/mês · Cobertura{" "}
                      {s.coverage > 365 ? "—" : `${s.coverage}d`}
                    </div>
                  </div>
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  Motivo: estoque abaixo do ponto de pedido ({s.reorder}) e cobertura inferior ao
                  lead time.
                </div>
              </div>
            ))}
            {suggestions.length === 0 && (
              <div className="text-sm text-muted-foreground">
                Sem necessidades — operação saudável.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <CardDescription>
          {canMove
            ...
            : rolesLoading
              ...
              : "Somente admin, gerente ou comprador podem mover cards."}
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {PCP_STAGES.map((stage) => {
            ...
            return (
              <div
                ...
                onDrop={(e) => {
                  if (!canMove) {
                    toast.error("Sem permissão para mover cards");
                    return;
                  }
                  e.preventDefault();
              ...
              >
                ...
                <div className="space-y-2 min-h-[40px]">
                  ...
                  {items.length === 0 && (
                    <div className="px-1 text-[11px] text-muted-foreground">—</div>
                  )}
                </div>
              </div>
            );
  ...
  const scored = (products as ProductRow[])
    .map((p) => {
      ...
      const score = Math.round(
        salesIdx * 0.35 + revenueIdx * 0.25 + turnover * 0.2 + Math.max(0, margin) * 0.2,
      );
      return {
        ...p,
        score,
      ...
      <CardContent className="grid gap-6 lg:grid-cols-3">
        {data.length === 0 ? (
          <div className="lg:col-span-3 rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">
            Registre vendas com UF na aba <strong>Vendas</strong> para ver a distribuição
            geográfica.
          </div>
        ) : (
          <>
            ...
            <div className="space-y-4">
              <div>
                <div className="mb-2 text-xs uppercase tracking-wide text-emerald-600">
                  Maior aceitação
                </div>
              ...
              <div>
                <div className="mb-2 text-xs uppercase tracking-wide text-red-500">
                  Baixa performance
                </div>
                <div className="space-y-1">
                  {bottom.map((s) => (
      ...
      <CardContent>
        {!hasData ? (
          <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">
            Sem vendas registradas ainda. Cadastre vendas na aba <strong>Vendas</strong> para ver a
            atribuição real por canal.
          </div>
        ) : (
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="h-72">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <KPI
          label="Alcance total"
          value={
            totals.reach >= 1_000_000
              ? `${(totals.reach / 1_000_000).toFixed(1)}M`
              : `${(totals.reach / 1000).toFixed(0)}k`
          }
          icon={TrendingUp}
        />
        ...
        <KPI
          ...
          value={
            totals.count
              ? `${totals.avgLift >= 0 ? "+" : ""}${Math.round(totals.avgLift * 100)}%`
              : "—"
          }
          icon={Target}
          tone="text-emerald-500"
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Cadastro completo, campanha e ROI antes × depois (dados persistidos)
            </CardDescription>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm" onClick={nu}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{draft.id ? "Editar influencer" : "Novo influencer"}</DialogTitle>
              </DialogHeader>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  <Label>Nome</Label>
                  <Input value={draft.nome ?? ""} onChange={(e) => field("nome", e.target.value)} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    maxLength={2}
                    value={draft.estado ?? ""}
                    onChange={(e) => field("estado", e.target.value.toUpperCase())}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    value={draft.seguidores ?? 0}
                    onChange={(e) => field("seguidores", Number(e.target.value))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    step="0.1"
                    value={draft.engajamento ?? 0}
                    onChange={(e) => field("engajamento", Number(e.target.value))}
                  />
                </div>
                <div>
                  <Label>Valor (R$)</Label>
                ...
                <div>
                  <Label>Vendas/dia ANTES</Label>
                  <Input
                    type="number"
                    value={draft.vendas_antes ?? 0}
                    onChange={(e) => field("vendas_antes", Number(e.target.value))}
                  />
                </div>
                <div>
                  <Label>Vendas/dia DEPOIS</Label>
                  <Input
                    type="number"
                    value={draft.vendas_depois ?? 0}
                    onChange={(e) => field("vendas_depois", Number(e.target.value))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    value={draft.ticket_medio ?? 180}
                    onChange={(e) => field("ticket_medio", Number(e.target.value))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    type="date"
                    value={draft.data_postagem ?? ""}
                    onChange={(e) => field("data_postagem", e.target.value)}
                  />
                </div>
              </div>
              <DialogFooter>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {items.map((i) => {
                  const va = Number(i.vendas_antes),
                    vd = Number(i.vendas_depois);
                  const lift = va > 0 ? ((vd - va) / va) * 100 : 0;
                  const receita = Math.max(0, vd - va) * 30 * Number(i.ticket_medio || 0);
                  const roi =
                    Number(i.valor) > 0 ? ((receita - Number(i.valor)) / Number(i.valor)) * 100 : 0;
                  return (
                    <TableRow key={i.id}>
                      <TableCell>
                      ...
                      <TableCell className="text-right tabular-nums">
                        {i.seguidores >= 1000
                          ? `${(i.seguidores / 1000).toFixed(0)}k`
                          : i.seguidores}
                      </TableCell>
                      <TableCell className="text-right tabular-nums">
                        {Number(i.engajamento).toFixed(1)}%
                      </TableCell>
                      <TableCell className="text-right tabular-nums">
                        {BRL(Number(i.valor))}
                      </TableCell>
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            ROI = (receita incremental − investimento) / investimento · Receita = (Δ vendas/dia) ×
            30 × ticket médio.
          </p>
        </CardContent>
      </Card>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {total} registros centralizados — base unificada que alimenta toda a inteligência
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Alimenta Production Intelligence, Geo Sales e Product Score com dados reais.
            </CardDescription>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label>SKU</Label>
                  <Input value={draft.sku ?? ""} onChange={(e) => field("sku", e.target.value)} />
                </div>
                <div>
                  <Label>Tamanho</Label>
                  <Input
                ...
                <div>
                  ...
                  <Input
                    maxLength={2}
                    value={draft.uf ?? ""}
                    onChange={(e) => field("uf", e.target.value.toUpperCase())}
                  />
                </div>
                <div>
                  <Label>Cidade</Label>
                ...
                <div>
                  ...
                  <Input
                    type="date"
                    value={draft.sold_at ?? ""}
                    onChange={(e) => field("sold_at", e.target.value)}
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => field("unit_price", Number(e.target.value))}
                  />
                </div>
              </div>
              <DialogFooter>
                ...
                <Button
                  ...
                  disabled={!draft.quantity || save.isPending}
                >
                  {save.isPending ? "Salvando..." : "Salvar"}
                </Button>
              </DialogFooter>
                ...
                {items.length === 0 && (
                  <TableRow>
                    <TableCell
                      ...
                      className="text-center text-sm text-muted-foreground py-8"
                    >
                      Nenhuma venda cadastrada. Clique em "Nova venda" para começar a alimentar a
                      inteligência.
                    </TableCell>
                  </TableRow>
                )}
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Velocidade calculada com vendas dos últimos {WINDOW_DAYS} dias · Cobertura-alvo{" "}
            {COVER_TARGET}d + lead time {LEAD_TIME}d.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
              ...
              <TableBody>
                {rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-right tabular-nums">
                      {r.daysCover >= 999 ? "∞" : `${Math.round(r.daysCover)}d`}
                    </TableCell>
                    <TableCell className="text-right tabular-nums font-semibold">
                      {r.suggested > 0 ? `${r.suggested} un` : "—"}
                    </TableCell>
                    <TableCell>
                      {r.level === "critical" && (
                ...
                {rows.length === 0 && (
                  <TableRow>
                    <TableCell
                      ...
                      className="text-center text-sm text-muted-foreground py-8"
                    >
                      Sem itens em estoque. Cadastre no Almoxarifado para iniciar a análise de
                      reposição.
                    </TableCell>
                  </TableRow>
                )}
