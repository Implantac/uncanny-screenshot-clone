...
function Cashflow() {
  ...
  const totals = useMemo(() => {
    ...
    const overdueValue = overdue.reduce(
      (s, a) => s + (a.type === "receber" ? a.value : -a.value),
      0,
    );
  ...
  const projection = useMemo(() => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const dayAccs = accs.filter(
        (a) => a.due_date === key && (a.status === "pendente" || a.status === "atrasado"),
      );
  ...
  const overdueList = useMemo(
    () =>
      accs
        .filter(
          (a) =>
            (a.status === "pendente" || a.status === "atrasado") && new Date(a.due_date) < today,
        )
        .slice(0, 50),
    [accs, today],
  );
    ...
    <div className="p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground">
          Saldo projetado, atrasos e próximas movimentações.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI
          label="A receber"
          value={fmt(totals.toReceive)}
          icon={<TrendingUp className="size-4" />}
          tone="success"
        />
        <KPI
          label="A pagar"
          value={fmt(totals.toPay)}
        ...
        <KPI
          label="Atrasos"
          value={`${totals.overdue} • ${fmt(Math.abs(totals.overdueValue))}`}
            ...
            {projection.map((p) => {
              ...
              return (
                <div
                  ...
                  title={`${p.d}: saldo ${fmt(p.running)}`}
                >
                  <div
                    className={`w-full ${positive ? "bg-success/40" : "bg-destructive/40"} rounded-sm`}
                    style={{ height: `${h}%`, marginTop: positive ? `${50 - h}%` : "50%" }}
                  />
                </div>
              );
            })}
          ...
          <div key={a.id} className="px-4 py-2 flex items-center justify-between text-sm">
            <div className="min-w-0">
              ...
              <div className="text-[10px] text-muted-foreground">
                ...
                {a.type === "receber" ? "Receber" : "Pagar"}
              </div>
            </div>
            <span
              className={`font-medium ${a.type === "receber" ? "text-success" : "text-destructive"}`}
            >
              {a.type === "receber" ? "+" : "-"}
              {fmt(a.value)}
            </span>
