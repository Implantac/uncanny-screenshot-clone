...
async function load() {
  const { data } = await supabase
    .from("products")
    .select("id, name, category, image_url, colors, sell_price")
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 rounded-xl border border-dashed border-border bg-card p-4 sm:p-6">
          ...
          {!preview && !mutation.isPending && (
            <button
              ...
              className="w-full flex flex-col items-center justify-center text-center gap-3 min-h-[160px] hover:bg-muted/30 transition rounded-lg"
            >
              <Upload className="size-8 text-muted-foreground" />
              <div>
                <div className="font-medium">Enviar referência visual</div>
                <div className="text-xs text-muted-foreground mt-1">
                  IA identifica categoria, estilo, cores e tendências (até 4MB).
                </div>
              </div>
            </button>
          )}
          {(preview || mutation.isPending) && (
            <div className="grid grid-cols-[120px_1fr] gap-4 items-start">
              ...
              <div className="min-w-0 space-y-2 text-xs">
                ...
                {result && (
                  <>
                    <div className="font-semibold text-sm">
                      {result.categoria} ·{" "}
                      <span className="text-muted-foreground font-normal">{result.estilo}</span>
                    </div>
                    ...
                    {result.tecidos.length > 0 && (
                      <div>
                        <span className="text-muted-foreground">Tecidos: </span>
                        {result.tecidos.join(", ")}
                      </div>
                    )}
                    {result.tendencias.length > 0 && (
                      <div className="flex gap-1 flex-wrap pt-1">
                        {result.tendencias.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 rounded-full bg-primary/15 text-primary text-[10px]"
                          >
                            {t}
                          </span>
                        ))}
                ...
                {mutation.error && (
                  <div className="text-destructive">{(mutation.error as Error).message}</div>
                )}
              </div>
            </div>
          )}
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="text-xs text-muted-foreground mb-2 flex items-center gap-2">
            <Palette className="size-3.5" /> Paleta predominante
          </div>
          <div className="grid grid-cols-8 gap-1">
            {palette.length === 0 ? (
              <div className="col-span-8 text-xs text-muted-foreground">Sem cores cadastradas.</div>
            ) : (
              palette.map(([c, n]) => (
                <div
                  key={c}
                  className="aspect-square rounded border border-border text-[9px] flex items-end justify-center pb-0.5 text-white/80 font-mono"
                  style={{ background: c, textShadow: "0 0 4px rgba(0,0,0,0.6)" }}
                  title={`${c} (${n})`}
                >
                  {n}
                </div>
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((c) => (
          <button
            key={c}
            onClick={() => setActive(c)}
            className={`text-xs px-3 py-1.5 rounded-full border transition ${active === c ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border text-muted-foreground hover:text-foreground"}`}
          >
            {c === "all" ? "Todos" : c}
          </button>
        ))}
