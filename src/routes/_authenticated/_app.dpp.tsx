...
type Passport = {
  ...
  cert: string;
  origem: string;
  col: { name: string; season: string; year: number } | null;
};
...
function DPP() {
  ...
  const avgCo2 = items.length
    ? (items.reduce((s, i) => s + Number(i.co2), 0) / items.length).toFixed(1)
    : "—";
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            l: "Passaportes emitidos",
            v: items.reduce((s, i) => s + i.emitidos, 0).toLocaleString("pt-BR"),
            i: QrCode,
          },
      ...
      {isLoading ? (
        ...
        <div className="glass rounded-xl p-12 text-center text-sm text-muted-foreground">
          Nenhum produto para emitir DPP. Cadastre produtos primeiro.
        </div>
      ) : (
            ...
            >
              ...
              <div className="mt-4 space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Peças emitidas</span>
                  <span className="tabular-nums font-medium">{p.emitidos}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">CO₂ por peça</span>
                  <span className="tabular-nums font-medium">{p.co2} kg</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Coleção</span>
                  <span className="font-medium truncate ml-2">
                    {p.col ? `${p.col.name} ${p.col.season}/${p.col.year}` : "—"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Certificações</span>
                  <span className="font-medium text-emerald-400">{p.cert}</span>
                </div>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <Field
                ...
                value={
                  passport.col
                    ? `${passport.col.name} ${passport.col.season}/${passport.col.year}`
                    : "—"
                }
              />
            ...
            <div className="glass rounded-lg p-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {STAGES.map((s, i) => (
                  <span
                    key={s}
                    className={`px-2 py-0.5 rounded-full text-[11px] ${i < STAGES.length - 1 ? "bg-emerald-500/15 text-emerald-400" : "bg-amber-500/15 text-amber-400"}`}
                  >
                    {s}
                  </span>
                ))}
...
function Field({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
      ...
      <div className={`mt-0.5 truncate ${accent ? "text-emerald-400 font-medium" : "font-medium"}`}>
        {value}
      </div>
    </div>
  );
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea
                ...
                rows={2}
                placeholder="Lavar a 30°C, não usar alvejante…"
              />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label>Reparabilidade (0-10)</Label>
                <Input
                  type="number"
                  min={0}
                  max={10}
