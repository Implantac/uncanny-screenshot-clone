    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="glass rounded-xl p-12 text-center text-sm text-muted-foreground">
          Nenhuma coleção para exibir. Crie uma coleção primeiro.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((l, i) => {
            ...
            return (
              <div
                key={l.id}
                className="glass rounded-xl overflow-hidden hover:border-primary/40 hover:-translate-y-0.5 transition-all group cursor-pointer"
              >
                <div
                  className={`relative h-44 bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]}`}
                >
                  {l.cover_url && (
                    <img
                      src={l.cover_url}
                  ...
                  <div className="absolute bottom-3 left-3 px-2 py-0.5 rounded bg-black/40 backdrop-blur text-[10px] text-white tabular-nums">
                    {l.looks} looks
                  </div>
                  <div className="absolute top-3 right-3 px-2 py-0.5 rounded bg-black/40 backdrop-blur text-[10px] text-white">
                    {l.season} {l.year}
                  </div>
                </div>
                <div className="p-4">
