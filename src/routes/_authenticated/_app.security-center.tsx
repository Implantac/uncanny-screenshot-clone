...
function SecurityCenter() {
  ...
  async function verifyEnroll() {
    ...
    if (cErr) {
      setLoading(false);
      return setMsg({ kind: "err", text: cErr.message });
    }
    ...
    setEnrolling(null);
    setCode("");
    setMsg({ kind: "ok", text: "MFA ativado com sucesso." });
    refresh();
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatusCard
          icon={ShieldCheck}
          label="MFA Ativo"
          active={verified.length > 0}
          hint={verified.length > 0 ? `${verified.length} fator(es)` : "Não configurado"}
        />
        ...
        <StatusCard
          ...
          label="Backup Diário"
          active
          hint="Gerenciado pela infra"
        />
      </div>

      {msg && (
        <div
          className={`rounded-md border p-3 text-sm ${msg.kind === "ok" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" : "border-rose-500/30 bg-rose-500/10 text-rose-700"}`}
        >
          {msg.text}
        </div>
      )}
          ...
          <div className="space-y-3 rounded-md border border-border p-4 bg-muted/30">
            ...
            <div className="flex gap-2">
              <input
                ...
                inputMode="numeric"
                maxLength={6}
                className="rounded-md border border-border bg-background px-3 py-2 text-sm font-mono w-40"
              />
              <button
