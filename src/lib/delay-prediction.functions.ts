...
const ORDER: Stage[] = ["cad", "corte", "costura", "acabamento", "qualidade", "expedicao", "entregue"];

// Fallback médio (em horas) quando não há histórico suficiente
const FALLBACK_H: Record<Stage, number> = {
  cad: 24, corte: 16, costura: 72, acabamento: 24, qualidade: 12, expedicao: 8, entregue: 0, concluido: 0,
};
...
export const predictDelays = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<{ items: DelayPrediction[]; avgPerStage: Record<string, number> }> => {
    ...
    const items: DelayPrediction[] = (orders ?? []).map((o: any) => {
      ...
      const remainRest = idx >= 0
        ? ORDER.slice(idx + 1, ORDER.indexOf("entregue")).reduce((s, st) => s + (avgH[st] ?? 0), 0)
        : 0;
      ...
      if (stuckH > Math.round((avgH[stage] ?? 0) * 1.5)) {
        reasonParts.push(`parada há ${stuckH}h em ${stage} (média ${Math.round(avgH[stage] ?? 0)}h)`);
      }
      if (idx >= 0 && idx < ORDER.indexOf("entregue") - 1) {
        reasonParts.push(`faltam ${Math.round(totalRemainH)}h estimadas até entrega`);
      }
      if (o.due_date) {
        reasonParts.push(predictedDelayH > 0 ? `previsão ${predictedDelayH}h após o prazo` : `dentro do prazo (${Math.abs(predictedDelayH)}h de folga)`);
      }
