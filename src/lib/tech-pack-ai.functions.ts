...
export const suggestTechSheetImprovements = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: sheet }, { data: mats }, { data: ops }] = await Promise.all([
      supabase.from("tech_sheets").select("id, code, materials_cost, labor_cost, cost_price, overhead_pct").eq("id", data.techSheetId).single(),
      supabase.from("tech_sheet_materials").select("description, consumption, unit, waste_pct, unit_cost, total_cost").eq("tech_sheet_id", data.techSheetId),
      supabase.from("tech_sheet_operations").select("description, sam_minutes, machine, unit_cost, total_cost").eq("tech_sheet_id", data.techSheetId),
    ]);
...
## BOM
${(mats ?? []).map((m: any) => `- ${m.description}: ${m.consumption}${m.unit ?? ""} × R$${m.unit_cost} (perda ${m.waste_pct ?? 0}%) = R$${m.total_cost}`).join("\n") || "(vazio)"}

## Operações
${(ops ?? []).map((o: any) => `- ${o.description}: ${o.sam_minutes ?? 0}min ${o.machine ?? ""} = R$${o.total_cost}`).join("\n") || "(vazio)"}

Gere o relatório agora.`;

    const res = await generateText({ model, system: SYSTEM, prompt, temperature: 0.5 });
