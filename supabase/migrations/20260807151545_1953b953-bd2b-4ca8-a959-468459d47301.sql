CREATE TYPE public.approval_workflow_status AS ENUM ('pendente','em_analise','aprovado','reprovado','pulado','cancelado');
...
BEGIN
  SELECT status INTO v_sheet_status FROM public.tech_sheets WHERE id = NEW.tech_sheet_id;

  IF NEW.status = 'reprovado' AND v_sheet_status = 'aprovada' THEN
    UPDATE public.tech_sheets
       SET status = 'em_revisao', approved_by = NULL, approved_at = NULL, approval_note = NULL
  ...
  IF NEW.status IN ('aprovado','pulado','em_analise') THEN
    ...
    FROM public.approval_workflow WHERE tech_sheet_id = NEW.tech_sheet_id;

    SELECT COUNT(*) INTO v_total FROM public.approval_workflow WHERE tech_sheet_id = NEW.tech_sheet_id;

    IF v_rejected > 0 THEN
      UPDATE public.tech_sheets
         SET status = 'em_revisao', approved_by = NULL, approved_at = NULL, approval_note = NULL
       WHERE id = NEW.tech_sheet_id AND status <> 'aprovada';
    ELSIF v_total > 0 AND v_done = v_total THEN
      UPDATE public.tech_sheets
         SET status = 'aprovada', approved_by = auth.uid(), approved_at = COALESCE(approved_at, now())
       WHERE id = NEW.tech_sheet_id AND status <> 'aprovada';
    END IF;
  END IF;
...
DECLARE
  v_owner uuid;
  v_product uuid;
  v_stages text[] := ARRAY['estilo','modelagem','compras','custos','qualidade','diretoria','producao'];
  v_idx int;
BEGIN
  SELECT owner_id, product_id INTO v_owner, v_product FROM public.tech_sheets WHERE id = _tech_sheet_id;
...
BEGIN
  UPDATE public.approval_workflow
     SET status = 'pendente', sent_at = NULL, decided_at = NULL, comment = NULL, assigned_to = NULL
   WHERE tech_sheet_id = _tech_sheet_id AND status IN ('pendente','em_analise');
END;
$$;