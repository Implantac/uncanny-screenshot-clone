...
BEGIN
  ...
  IF NEW.status = 'lancamento' THEN
    ...
    IF NOT EXISTS (
      ...
      -- Auto-vincular ao produto carro-chefe (role=hero, menor display_order)
      SELECT cp.product_id INTO hero_pid
        FROM public.collection_products cp
       WHERE cp.collection_id = NEW.id
         AND cp.owner_id = NEW.owner_id
         AND cp.role = 'hero'
      ...
      INSERT INTO public.marketing_briefs(
        owner_id, collection_id, product_id, title, objective, target_audience,
        key_message, tone, channels, status, lifecycle_trigger
      ) VALUES (
        NEW.owner_id, NEW.id, hero_pid,
        'Lançamento: ' || NEW.name,
        ...
        'Apresentar a nova coleção com identidade da temporada' ||
          CASE WHEN hero_pid IS NOT NULL THEN ' — destaque para o produto carro-chefe' ELSE '' END,
        'aspiracional',
      ...
      SELECT us.user_id, 'collection_launch_brief',
             ...
             'Coleção entrou em lançamento — revise o brief gerado'
               || CASE WHEN hero_pid IS NOT NULL THEN ' (carro-chefe vinculado)' ELSE '' END,
             '/marketing', NEW.id
