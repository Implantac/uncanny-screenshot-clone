...
test.describe("ProductReadinessBadge — aria-describedby (E2E)", () => {
  ...
  for (const state of ["ready", "pending"] as const) {
    test(`[${state}] aria-describedby aparece ao abrir e some ao fechar`, async ({ page }) => {
      const trigger = variants[state];
      test.skip(!trigger, `no ${state} badge in dataset`);

      await trigger!.scrollIntoViewIfNeeded();

      // Estado inicial: sem aria-describedby (ou vazio).
      const initial = await trigger!.getAttribute("aria-describedby");
      expect(initial ?? "").toBe("");

      // Abre via foco de teclado.
      await trigger!.focus();

      // aria-describedby deve aparecer e apontar para o role="tooltip" visível.
      const describedBy = await expect
      ...
      // Fecha via Escape — aria-describedby deve sumir (ou ficar vazio).
