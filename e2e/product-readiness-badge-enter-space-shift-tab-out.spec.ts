...
test.describe("ProductReadinessBadge — Enter/Espaço abre, Shift+Tab fecha (E2E)", () => {
  ...
  for (const state of ["ready", "pending"] as const) {
    for (const key of ["Enter", "Space"] as const) {
      test(`[${state}] ${key} abre tooltip; Shift+Tab tira o foco, tooltip some e aria-describedby é removido`, async ({
        page,
      }) => {
        ...
        // Shift+Tab move o foco pra fora do trigger (elemento anterior).
