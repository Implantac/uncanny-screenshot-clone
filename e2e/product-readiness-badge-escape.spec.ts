...
test.describe("ProductReadinessBadge — Escape remove aria-describedby (E2E)", () => {
  ...
  for (const state of ["ready", "pending"] as const) {
    test(`[${state}] foco abre tooltip; Escape remove aria-describedby e esconde tooltip`, async ({
      page,
    }) => {
      ...
      // Tooltip abre e aria-describedby aponta pro role="tooltip" com conteúdo esperado.
      await expect
      ...
      // Escape → tooltip some, aria-describedby removido, foco preservado.
